...
.menu-button, .utility-nav button, .hero-actions a, .card-actions a, .audience-tabs a, .resource-grid a {
  border: 1px solid var(--border);
  border-radius: 8px;
...
p { color: var(--text); }
.lede { color: var(--muted); font-size: 1.08rem; max-width: 66ch; }
.eyebrow {
  margin: 0 0 0.35rem;
  color: var(--accent-strong);
  font-size: 0.78rem;
...
.hero-actions a:first-child, .audience-tabs a[aria-current="page"] {
  background: var(--accent-strong);
  color: #fff;
...
@media (max-width: 820px) {
  ...
  .feature-list, .metric-grid, .group-grid, .glossary-grid, .meta-grid { grid-template-columns: 1fr; }
