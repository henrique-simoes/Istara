...
def design_markdown() -> str:
    return """# Istara Documentation Website Design

This file is generated with the feature documentation site so the public website
keeps a durable design contract beside the deployed artifacts.

## Purpose

The website is Istara's public entry point. It must help visitors quickly
understand what Istara is, install it, and move into the feature documentation
without feeling like they have landed inside an internal engineering artifact.

## Visual Direction

- Use the Istara logo as the primary visual anchor.
- Match the page background to the logo background color: `#bdb6a4`.
- Keep the hero quiet, editorial, and product-focused rather than oversized or
  decorative.
- Avoid novelty gradients, decorative orbs, and type sizes that make the page
  feel distorted.
...
def generate_site(inventory: dict[str, Any]) -> list[Path]:
    ...
    design_path.write_text(design_markdown(), encoding="utf-8")
