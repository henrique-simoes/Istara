...
async def webauthn_register_start(
    ...
    if token_data.get("sub") != user.id:
        raise HTTPException(
            status_code=403, detail="Cannot register a passkey for another user"
        )
...
async def webauthn_register_finish(
    ...
    try:
        ...
        verification = verify_registration_response(
            credential_id=(
                base64url_to_bytes(body.raw_id)
                if body.raw_id
                else base64url_to_bytes(body.id)
            ),
...
async def webauthn_authenticate_finish(
    ...
    try:
        ...
        if credential.user_id != body.user_id:
            raise HTTPException(
                status_code=401, detail="Credential does not match challenge user"
            )
