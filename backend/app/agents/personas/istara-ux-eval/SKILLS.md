...
- First-time user experience: what happens when a new user opens Istara?
...
- Assess whether Istara's complexity is justified by its capabilities relative to simpler alternatives
...
- Competitive positioning analysis (where Istara leads, where it lags)
...
- First-run experience: after install, user sees the tray icon or desktop companion and immediately understands whether this machine is a Server or a Client. Evaluate: is the path from "just installed" to "open browser and see Istara" obvious (Goal-Gradient Effect)? If the installer needed Homebrew, was Homebrew explained in plain language first? Does auto-start on login match user expectations (Jakob's Law — similar apps auto-start)? The tray now provides immediate feedback on every action — Start/Stop shows actual port state in menu labels (Visibility of System Status), Compute Donation toggle always shows confirmation dialog (Error Prevention), and Check Updates always shows result (Closure).
- Dependency choice UX: users choose Python, Node, Ollama, LM Studio. Evaluate: are descriptions clear for non-developers? Does the installer explain the difference between LM Studio and Ollama well enough that a user can choose confidently? Are first-model instructions clear enough that a user can get from install to first response without external docs?
...
- Connection string onboarding friction: evaluate the copy-paste flow — admin generates a string in Settings, shares it out-of-band (email, Slack, etc.), and the new user pastes it into the desktop app or "Join Server" tab. Assess: is the copy button discoverable for admins (Recognition Rather Than Recall)? Is the paste-and-connect action a single step or does it require additional confirmation? Does the product explain that the invite already contains the server address so the user does not need to configure URLs manually?
- Installer UX for non-developers: evaluate the macOS .dmg and Windows .exe first-run experience. The dependency checker and .env wizard must be understandable by researchers with no terminal experience. Assess: does the wizard use plain language ("Where should Istara store data?" not "Set DATA_DIR path")? Is the Server+Client vs Client-only choice explained with use-case labels ("I'm setting up a team server" vs "I'm joining an existing team")? Does the install complete in under 5 minutes for the common case (Doherty Threshold)?
- Desktop app system tray discoverability: evaluate whether non-technical users understand that Istara runs in the system tray after install. Assess: is there a first-launch notification explaining the tray icon (Paradox of Active User — users may close the window and think the app quit)? Tray menu now provides rich feedback: Start/Stop shows actual running state with `●`/`○` indicators (Visibility of System Status), Client mode opens the configured remote workspace, every action (Donate, LLM click, Check Updates) triggers a native dialog with result (Closure — users always know what happened), and Client mode blocks donation until an invite exists. Errors show log details. Evaluate: is the underlying process-management architecture invisible to users (Tesler's Law — complexity is in the system, not the UX)?
