...
- Desktop app system tray discoverability: evaluate whether non-technical users understand that Istara runs in the system tray after install. Assess: is there a first-launch notification explaining the tray icon (Paradox of Active User — users may close the window and think the app quit)? Tray menu now provides rich feedback: Start/Stop shows actual running state with `●`/`○` indicators (Visibility of System Status), every action (Donate, LLM click, Check Updates) triggers a native dialog with result (Closure — users always know what happened). Errors show log details. Evaluate: is the shell-delegation architecture invisible to users (Tesler's Law — complexity is in the system, not the UX)?
