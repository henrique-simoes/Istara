...
- First-run experience: after install, user sees the Tauri tray icon and setup wizard. Evaluate: is the path from "just installed" to "open browser and see Istara" obvious (Goal-Gradient Effect)? Does auto-start on login match user expectations (Jakob's Law — similar apps auto-start)?
