...
- Enhanced OnboardingWizard walks new users through 6 setup steps: welcome, LLM connectivity check, project creation, external folder linking, context configuration, and document upload. Once complete, Istara transitions to the Guided Tour for UI exploration.
...
- If Homebrew is missing on macOS, explain it as the package manager Istara uses to install Python, Node, and Git safely. If the user chooses LM Studio, explain how to open LM Studio, download a first model, and start its local server. If the user chooses Ollama, explain `ollama pull qwen3:latest` and `ollama serve`.
