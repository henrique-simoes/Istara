...
- **Connection string round-trip**: Admin generates a connection string, verify the response is `rcl_`-prefixed and HMAC-signed. Decode the string, extract `server_url`, `ws_url`, network token, JWT, label, and expiration. Verify `server_url` remains the web app origin and `ws_url` points to `/ws/relay`. Paste the string into the client setup flow, verify the client authenticates and receives a session. Test with tampered, expired, revoked, redeemed, and rate-limited validation attempts — verify specific errors, not one generic failure.
