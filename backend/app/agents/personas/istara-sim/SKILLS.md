...
- **Relay capability detection**: Register a relay node with a validated connection string or authenticated browser JWT, then verify the server records the relay, keeps the websocket alive through heartbeat, and populates cached model/capability metadata when the provider is reachable. Browser donors are not assumed to be reachable by backend HTTP; their advertised model list and websocket liveness are the source of truth.
...
- **Relay execution path**: Register a relay that reports `provider_host: "http://localhost:1234"`. Verify the backend does not depend on direct backend-to-donor HTTP for chat or embedding completion: it sends `llm_request` / `embed_request` over `/ws/relay`, receives the matching response, resolves the pending request, and clears it. Verify streaming falls back to the same websocket request path and yields the returned content. Simulate timeout and disconnect paths to confirm pending requests are failed and cleared.
- **Relay host resolution**: Register a CLI relay that reports `provider_host: "http://localhost:1234"`. Verify the backend resolves localhost to the relay's actual IP for capability detection only. A failed provider probe must not prevent websocket chat execution when the relay is alive.
...
- **All-node model listing**: Add a local server, a network/admin-added server, a CLI relay, and a browser donor. Verify `/settings/models` and `/compute/stats` list models from every healthy node with `server_name`, `provider_type`, and source context so admins can see which machine/model is serving work.
...
- **OnboardingWizard step progression**: Trigger the wizard on a fresh installation. Walk through all 6 steps (welcome, LLM check, project, folder link, context, upload). Verify each step transition works, back navigation returns to the previous step, and skipping optional steps does not block completion. Once the 6-step Wizard is complete, verify Istara transitions to the Guided Tour for UI exploration. Test closing the wizard mid-flow and reopening — verify it resumes at the correct step or restarts gracefully.
...
- **Linked folder document sync resolution**: Link an external folder via `POST /projects/{id}/link-folder`, place a test file (e.g., `external-test.txt`) in it, then call `POST /api/documents/sync/{id}`. Verify the sync endpoint scans the linked `watch_folder_path` (not the internal uploads dir) and registers the file. Verify the simulation scenario 29-documents-system.mjs check 14b covers this: create temp dir → link → sync → assert synced >= 1 → unlink → cleanup.
...
- **CI/CD artifact verification**: Push a release-worthy change to `main`, verify GitHub Actions builds both macOS DMG and Windows EXE and creates a GitHub Release for that commit. Push a non-release-worthy change to `main`, verify the release workflow does not publish. Push a `v*` tag, verify GitHub Release creation still works for the tag path as well.
...
- **Connection string round-trip**: Admin generates a user invite connection string, verify the response is `rcl_`-prefixed and HMAC-signed. Decode the string, extract `server_url`, `ws_url`, nonce, intended role, label, and expiration, and verify no pre-minted login JWT is present. Verify `server_url` remains the web app origin and `ws_url` points to `/ws/relay`. Paste the string into the client setup flow, verify redemption creates a user, returns one-time recovery codes, and receives a server-backed session. Test with tampered, expired, revoked, redeemed, and rate-limited validation/redemption attempts — verify specific errors, not one generic failure.
- **Direct URL rejection**: Try to paste `http://server:3000`, `https://server`, and a bare IP into client setup. Verify the installer/desktop client reject the value as an invite string, do not save it as a trusted remote, and show guidance to use an admin-generated `rcl_` string.
...
- **Browser compute relay**: Simulate the DonateComputeToggle flow with a valid JWT. Verify a WebSocket relay connection is established, the relay node appears in the compute registry with source type `browser`, LM Studio and Ollama chat/embedding responses are normalized, donor-side errors are surfaced in the UI, and toggling off removes the node. Test toggling on without a local LLM — verify a graceful "No local LLM detected" response, not an error.
- **Relay --connection-string bootstrap**: Simulate relay startup with a connection string flag: decode the string, verify server-side validation succeeds, extract `ws_url`, and verify the relay connects to the correct WebSocket endpoint with the `X-Access-Token` header. Test with an invalid connection string — verify the relay exits with a descriptive error, not a crash. Test with a valid string pointing to an unreachable server — verify timeout and retry behavior.
- **Installer first-run simulation**: Simulate the dependency checker: mock missing Python/Node dependencies and verify the checker reports them with actionable install instructions. For the shell installer, verify Server vs Client mode copy explains what will be installed, Homebrew is explained before installation, LM Studio vs Ollama choice includes first-model guidance, the install syncs the source checkout cleanly from `main`, macOS always attempts to install the desktop companion app, and Client-only mode writes `~/.istara/config.json` even before an invite is pasted.
- **Desktop app lifecycle (mode-aware desktop companion)**: Simulate Tauri tray interactions: Start menu item starts local backend/frontend and label changes to `● Stop Istara Server` when ports are up. Stop changes label to `○ Start Istara Server`. Toggle Compute Donation on — verify relay starts, config saves, confirmation dialog appears. Toggle off — verify relay stops, config saves, dialog appears. In Client mode with a saved connection string, "Open Istara" must open the remote server URL. In Client mode without a connection string, Donate must block with guidance to use "Change Server / Invite...". Click LM Studio when running — verify status dialog with donation toggle option. Click when not running — verify LM Studio launch offer. Check for Updates — verify three-tier check (Tauri updater, git tags, GitHub releases) always shows a result dialog and only opens releases when the user confirms.
...
- **Login UX flow**: Local mode — verify name-only screen appears, "Get Started" works, JWT is issued. Team mode with no users — verify registration screen auto-appears, first user gets admin role, and the app does not briefly render the main screen before registration. Server unreachable — verify error screen with `istara start` instructions. Team mode with users — verify login/register/join toggles work.
...
- **Guided tour flow**: Simulate the full 16-step admin onboarding flow: 6 Wizard steps (configuration) followed by 10 Tour steps (UI navigation: folder input → project creation → team mode toggle → connection string generation → file prompt → context fill → tasks view → LLM check → chat arrival). Verify step progression, conditional skipping (non-admin skips steps 2-4, existing projects skip 0-1), localStorage persistence across refresh, and skip functionality. Add a regression check for stale `istara_tour_completed` in localStorage: on a fresh system with zero projects, onboarding must still appear.
