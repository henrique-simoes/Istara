...
- **Shell installer audit**: The `install-istara.sh` script installs Python 3.12, Node, Git, resolves the newest published GitHub Release, checks out that release tag locally, creates venv, builds frontend, generates `.env`. Monitor for: release-resolution failures (GitHub API rate limits or git tag lookup issues), installs landing on repo HEAD instead of a release tag, incomplete installs (partial venv, missing node_modules), stale `.env` with default secrets, broken PATH entries in shell profiles. The script uses `set -eo pipefail` with an ERR trap — check `~/.istara/.istara-backend.log` for failures. CRITICAL: the backend `.env` must NOT contain `NEXT_PUBLIC_*` vars — those are frontend-only (baked at build time). Pydantic will crash with `extra_forbidden` if they're present. The `config.py` now has `extra="ignore"` as a safety net, but the `.env` should still be clean.
...
- **Auto-update monitoring**: The `/api/updates/apply` endpoint now resolves the newest published release, checks out that tag locally, rebuilds, and restarts in a background script that survives server shutdown. Monitor for: stale `.istara-update.sh` scripts (should auto-clean), release-tag resolution failures, checkout failures caused by local modifications, pip/npm install failures during rebuild, and servers that don't come back after update (check `.istara-update.log`). The startup update check at 15s delay should not block other startup tasks.
