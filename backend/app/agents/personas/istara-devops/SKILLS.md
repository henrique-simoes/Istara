...
- Relay auth layers: relay connections authenticate via X-Access-Token header (network token) plus the embedded JWT from the connection string. The relay decodes the payload to locate the server; the server enforces network token/JWT validity because clients do not have the HMAC secret. Monitor for: relays connecting with mismatched token/JWT pairs, connection string decoder failures, and unauthorized relay registration attempts.
