...
- Desktop app process monitoring: Tauri v2 tray app runs in Server+Client or Client-only mode. ProcessManager now verifies actual process state via `try_wait()` before reporting is_running, so zombie processes are properly cleaned up. Logs go to `~/.istara/logs/{backend,frontend,relay}.log` for debugging. Monitor for: stale Child objects after crashes (should auto-clean on next is_running call), log file growth, corrupt config.json (now backed up to .json.bak before reset). The health loop rebuilds the tray menu only on state change or every 30s (was every 10s). GitHub API update checks include rate-limit detection and User-Agent header.
