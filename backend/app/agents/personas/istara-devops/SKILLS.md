...
- Verify global auth middleware is active: in Team Mode, all non-exempt endpoints must return 401 without JWT; in Local Mode, local-first desktop requests use the built-in local admin identity.
...
- Relay WebSocket execution: relay/browser donors serve chat and embedding requests through authenticated `llm_request` / `embed_request` messages and matching responses. Monitor for: request timeouts, disconnects during active requests, pending request maps that do not clear, and repeated donor-side errors.
- Relay host resolution: when a CLI relay node reports `provider_host: "http://localhost:1234"`, the backend resolves localhost to the relay's actual IP for optional capability probes. Chat/embedding serving must continue over WebSocket even if that probe fails.
- Relay capability detection: health checks can probe reachable CLI relay nodes via HTTP for model capabilities. Browser donors rely on heartbeat and advertised model lists because backend HTTP cannot reach browser-local providers. Monitor: stale capability data, stale model lists, and probe failures that are incorrectly treated as serving failures.
...
- Connection string security: `rcl_`-prefixed user invite strings are HMAC-SHA256 signed bundles containing server URL, WebSocket URL, nonce, intended role, and expiration. They do not carry pre-minted login JWTs; redemption creates the user and server-backed auth session. Monitor for HMAC validation failures, expired/redeemed strings, and brute-force paste attempts on the "Join Server" endpoint. Rate-limit connection string redemption to prevent credential stuffing.
...
- Rate limiter identity: login and connection-string validation rate limiters use proxy-aware client IP extraction (`X-Forwarded-For`, `X-Real-IP`, then socket host) and bounded in-memory buckets. Monitor for: trusted proxy misconfiguration, spoofed forwarded headers on direct internet exposure, and excessive bucket eviction under attack.
- Relay auth layers: compute-donation strings carry relay bootstrap data and relay connections authenticate via the X-Access-Token header. The relay decodes the payload to locate the server, while the server enforces network-token validity and connection-string issuance state because clients do not have the HMAC secret. Monitor for stale tokens after rotation, connection string decoder failures, and unauthorized relay registration attempts.
- Remote API origin: production frontend builds should leave `NEXT_PUBLIC_API_URL` / `NEXT_PUBLIC_WS_URL` unset unless a public domain is explicitly configured, so runtime host derivation points remote browsers at the server instead of their own localhost. Monitor failed remote join attempts for localhost API URLs in browser network traces.
- Connection string URL split: monitor generated invite payloads for `server_url` pointing to the UI origin and `ws_url` pointing to `/ws/relay`. If `server_url` points at port 8000, desktop Client mode will open the backend instead of the app.
- Relay execution path: donated compute routes through WebSocket request IDs and `llm_response` / `embed_response` messages. Monitor for pending relay request timeouts, relays registered with empty model lists, donor embedding failures, and health probes that fail even though WebSocket routing still works.
...
- Browser compute donation audit: DonateComputeToggle opens a WebSocket relay from the browser and surfaces donor-side errors in the UI. Monitor for: relay connections from unauthenticated browser sessions (should require valid JWT), excessive relay registrations from a single user, local provider CORS/fetch failures, and stale browser model lists.
