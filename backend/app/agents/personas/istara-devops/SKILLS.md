...
- Desktop tray app (Rust/Tauri v2): delegates start/stop to `istara.sh` via shell delegation (single source of truth for PID management). Only the relay daemon is managed directly in Rust. Monitor for: `istara.sh` not found (empty install_dir), shell execution failures on macOS sandboxed apps, ANSI color codes leaking into error dialogs.
...
- Desktop app process monitoring: Tauri v2 tray app delegates start/stop to `istara.sh` (shell delegation architecture). The CLI manages PID files, process groups, port cleanup, and health waits — the tray is a thin GUI layer. Only the relay daemon is managed directly in Rust with `try_wait()` zombie detection. Relay logs go to `~/.istara/logs/relay.log`. Menu labels are driven by port checks (8000, 3000, 1234, 11434) and update after every action. Monitor for: `istara.sh` not found, shell delegation failures, ANSI escape codes in error dialogs (strip_ansi handles this). Health loop rebuilds tray menu on state change or every 30s. Update checks use `git fetch --tags` (no API rate limit). Compute Donation toggle always saves config and shows feedback dialog. LLM Status click offers donation toggle or LM Studio launch. Check Updates uses three-tier approach: Tauri updater, git tags, GitHub releases — all paths show result dialog. Corrupt config.json is backed up to .json.bak before reset.
