...
- Color token usage: all UI elements should use design system tokens (istara-50 through istara-950), not raw hex/rgb
...
- Evaluate the Integrations view: tab navigation, consistency with other Istara views (Interfaces, Skills, etc.)
...
- System tray menu: verify keyboard accessibility for tray menu items on both macOS (menubar) and Windows (system tray). Ensure tray tooltip updates are announced by screen readers. Menu labels now show live state indicators (● for running, ○ for stopped, ✓ for connected, ⚠ for warnings). Verify these unicode indicators are accessible to screen readers. Feedback dialogs (donation toggle, update results, LLM status, errors) must be readable by screen readers with proper `role="dialog"` semantics.
...
- Desktop app tray menu: audit the Tauri v2 system tray menu for keyboard accessibility — menu must open on click and keyboard activation, items must be reachable via Arrow keys. Start/Stop label changes dynamically based on port state (`● Stop Istara Server` vs `○ Start Istara Server`). Compute Donation shows `✓ Compute Donation: On` vs `○ Compute Donation: Off`. Every action (donate, LLM click, update check) triggers a native feedback dialog — verify these are keyboard-dismissible and screen-reader-announced. Error dialogs from failed operations show log details in readable format (ANSI codes stripped).
