...
class ContextDAGNode(Base):
    ...
    message_ids: Mapped[str] = mapped_column(
        Text, default="[]"
    )  # JSON array of original message IDs covered
    child_node_ids: Mapped[str] = mapped_column(
        Text, default="[]"
    )  # JSON array of direct child node IDs
    token_count: Mapped[int] = mapped_column(Integer, default=0)  # tokens in summary
    original_token_count: Mapped[int] = mapped_column(
        Integer, default=0
    )  # tokens in original messages
    message_count: Mapped[int] = mapped_column(
        Integer, default=0
    )  # total original messages covered
