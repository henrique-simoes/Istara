...
class ResearchDeployment(Base):
    ...
    config_json: Mapped[str] = mapped_column(
        Text, default="{}"
    )  # adaptive rules, branching, rate limits
    channel_instance_ids_json: Mapped[str] = mapped_column(
        Text, default="[]"
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "channel_instance_ids": json.loads(self.channel_instance_ids_json)
            if self.channel_instance_ids_json
            else [],
            "state": self.state,
