...
class SimulationSkill(BaseSkill):
    ...
    def display_name(self) -> str:
        return "Game Theory Participant Simulation"

    @property
    def description(self) -> str:
        return "Test research designs against game-theory personas (Skeptic, Satisficer, etc.)."
    ...
    def skill_type(self) -> SkillType:
        return SkillType.QUANTITATIVE

    async def plan(self, skill_input: SkillInput) -> dict:
        return {
            "steps": [
                "Initialize participant cohort",
                "Define game-theory scenario",
    ...
    async def execute(self, skill_input: SkillInput) -> SkillOutput:
        ...
        p1 = ParticipantProfile(
            id="P1", name="The Skeptic", strategy=ParticipantStrategy.ADVERSARIAL, patience=0.4
        )
        p2 = ParticipantProfile(
            id="P2",
            name="The Satisficer",
            strategy=ParticipantStrategy.SATFICING,
            engagement_level=0.3,
        )
        p3 = ParticipantProfile(
            id="P3", name="The Idealist", strategy=ParticipantStrategy.COOPERATIVE, honesty_bias=1.0
        )
        ...
        return SkillOutput(
            ...
            suggestions=[
                "Refine the prompt to reduce social desirability bias in 'The Idealist'.",
                "Add more constraints to prevent low-effort responses from 'The Satisficer'.",
            ],
        )
