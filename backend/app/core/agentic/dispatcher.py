...
def _request_text(
    system: str | None, messages: list[dict[str, Any]], extra: str | None = None
) -> str:
    parts = [system or ""]
...
class AgenticDispatcher:
    ...
    def __init__(
        ...
        *,
        ...
        embeddings_gateway: Any | None = None,
    ) -> None:
    ...
    async def completion(
        ...
        *,
        ...
        try:
            if selected == "pi":
                outcome = await self._pi.run_completion(
                    purpose=purpose,
                    project_id=project_id,
                    agent_id=agent_id,
                    system=system or "",
        ...
        *,
        ...
        try:
            if selected != "pi":
                ...
                outcome = await self._pi.run_react(
                    purpose=purpose,
                    project_id=project_id,
                    agent_id=agent_id,
                    session_key=session_key,
    ...
    async def ensemble(
        ...
        *,
        ...
        try:
            if selected == "pi":
                outcome = await self._pi.run_ensemble(
                    purpose=purpose,
                    project_id=project_id,
                    agent_id=agent_id,
                    system=system or "",
    ...
    async def embed(
        ...
        *,
        ...
        """Embed through Pi Model Management for either loop-mode selection."""
        params = params or TurnParams()
        started = time.perf_counter()
        selected = await self._resolve(project_id=project_id, engine=engine, request=request)
        if selected in ("pi", "legacy"):
            # Engine selection governs loop semantics, not model/provider
            # authority. Embeddings have no loop and therefore always use the
            # one Pi-managed gateway; they never leak to ComputeRegistry.
            try:
    ...
    def steering_binding(
        ...
        """
        return self._pi.steering_binding(
    ...
    async def _record_failure(
        ...
        *,
        ...
        spine_phase: str | None,
        session_id: str | None = None,
        exc: Exception = RuntimeError("unknown"),
    ) -> None:
    ...
    async def _legacy_outcome(self, verb: str, **kwargs: Any) -> dict[str, Any]:
        ...
        kwargs.setdefault("provider_service", self._pi)
        if verb == "embed":
            kwargs.setdefault("embeddings_gateway", self._embed_gateway())
        return await self._legacy(verb=verb, **kwargs)


async def _no_tool_executor(
