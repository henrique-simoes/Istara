...
ADMIN_SURFACES = {"backend_code", "integrations", "mcp", "compute", "security", "connection_strings"}
...
class SandboxEvaluationService:
    ...
    def evaluate_proposal(self, proposal: Any, *, apply_evidence: dict | None = None) -> dict:
        ...
        evaluation_runs = proposal.get_evaluation_runs() if hasattr(proposal, "get_evaluation_runs") else []
        reasoning_memory_ids = (
            proposal.get_reasoning_memory_ids() if hasattr(proposal, "get_reasoning_memory_ids") else []
        )
        return self.evaluate_payload(
            ...
            affected_surfaces=proposal.get_affected_surfaces() if hasattr(proposal, "get_affected_surfaces") else [],
            ...
            proposed_change=proposal.get_proposed_change() if hasattr(proposal, "get_proposed_change") else {},
            rollback_plan=proposal.get_rollback_plan() if hasattr(proposal, "get_rollback_plan") else {},
            evidence=evidence,
            evaluation_runs=evaluation_runs,
            reasoning_memory_ids=reasoning_memory_ids,
            metrics_before=proposal.get_metrics_before() if hasattr(proposal, "get_metrics_before") else {},
            metrics_after=proposal.get_metrics_after() if hasattr(proposal, "get_metrics_after") else {},
    ...
    def evaluate_payload(
        ...
        *,
        ...
        def add_check(check_id: str, passed: bool, severity: str, message: str, detail: Any = None) -> None:
            checks.append(
                {
                    "id": check_id,
                    "passed": passed,
        ...
        rollback_text = " ".join(str(value).lower() for value in rollback.values()) if isinstance(rollback, dict) else ""
        has_evaluation_evidence = bool(runs or before or after or apply_payload.get("command") or apply_payload.get("tests"))
        ...
        add_check(
            "rollback_actionable",
            bool(rollback_text and any(token in rollback_text for token in ("restore", "revert", "disable", "delete", "remove", "quarantine", "revoke"))),
            "warning",
        ...
        add_check(
            "secret_redaction",
            "[REDACTED]" not in str({"proposed": proposed, "rollback": rollback, "apply": apply_payload}),
            "warning",
