...
### Changed
- **CI Failure-Domain Independence + Required-Check Contract**: `.github/workflows/ci.yml` redesigned so formatting (`backend-format`), correctness lint (`backend-lint`: repo-wide `F821,F811,F822` blocking + changed-file strict gate + advisory style backlog), tests (`backend-test` incl. QA contract tests), mutation (`backend-mutation`), frontend lint/typecheck/unit/mutation/build, governance, JS harnesses, QA compose render (`qa-contract-render`, honestly named), and a container-first `ui-journeys` browser lane each report independently — a red format check can no longer hide the test suite — with a fail-closed `release-gate` aggregator that fails on failure, cancellation, and unexplained skip. Required-check semantics now live in the committed `testing/required-checks.json` manifest, contract-checked every run by `scripts/check_required_checks.py`; that manifest is the verbatim source for the owner-gated `main` branch-protection package prepared (never applied by agents) in `docs/promotion/branch-protection/`. CI jobs are now write-free (`governance` is `contents: read`); the README badge writeback moved to the narrow main-only `.github/workflows/badge-sync.yml` (`[skip ci]` commits). Frontend mutation: `related: false` for honest no-coverage counting plus an advisory-first widened `src/lib/*Api.ts` scope (`npm run test:mutation:wide`, thresholds unchanged). `desktop-check` reports honestly without `continue-on-error`; its required-context status is recorded as an owner decision (M-18). The `ui-journeys` lane starts `docker-compose.qa.yml --profile ui` loopback-only with synthetic data, runs the simulation harness, uploads SHA-keyed evidence, and handles `not_runnable` honestly: registered-but-unproven journeys are recorded explicitly and missing capabilities fail the job with a named reason.
