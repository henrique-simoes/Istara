...
| Simulation scenarios | `tests/simulation/scenarios/*.mjs` | Numbered scenario files (numbers 10 and 70 are reused; gaps exist) plus static helper tests in `lib/*.test.mjs`. CI `ui-journeys` runs only `testing/ui-journeys.smoke.json` + `proven-extra.json`; `npm test` runs everything against a running app. | Browser/API scenarios against a running app, registered through `tests/simulation/run.mjs`; `npm run test:static` is the PR-safe syntax/project-selection smoke layer. |
