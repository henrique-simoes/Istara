...
| --- | --- | --- | --- |
| Python pytest suite | `tests/` | 2384 collected tests (1 deselected; 2383 run) at the 2026-09-11 final certification on `25e2063d` with the exact CI command (`cd backend && pytest ../tests/ -q --tb=short -m "not live_llm"`, keyless, py3.12.13, both pi surfaces installed): 2377 passed / 6 skipped / 1 deselected, 0 failed. There are 164 `test_*.py` files directly under `tests/` plus suites under `tests/pi_production/`, `tests/pi_benchmark/`, `tests/pi_migration/`, `tests/pi_compat/`, `tests/benchmarks/`, and `tests/integration/`; some files are splitters/pointers and `tests/e2e_test.py` is a standalone live script. | Default deterministic contract, security, governance, backend, and cross-surface coverage. |
...
| Frontend unit tests | `frontend/src/**/*.test.ts` | 23 test files, 121 tests at the 2026-09-11 final certification on `25e2063d` (node 24.20.0; runtime config, model providers/catalog, forms, navigation, OAuth poll lifecycle, cookie-tolerant auth custody). Frontend mutation on the same SHA: 90.77 (118 killed / 12 survived / 0 no-cov) ≥ break 75. | Run with `npm run test:unit`. |
...
| Simulation scenarios | `tests/simulation/scenarios/*.mjs` | 86 scenario files plus 11 static helper test files under `tests/simulation/lib/*.test.mjs` at the 2026-09-26 checkpoint (test:static 41/41; browser lane 2026-09-26: 22 scenarios, 318/320 checks on the first pass, both remaining failures fixed and re-run green: 85 11/11, 88 11/11). The shared 375 px check (`reflow375Check`) fails when anything inside `main` is cut off, not only when the page scrolls sideways; scenario 88 applies it to every shell view. Earlier: 2026-09-11 15 PASS + 1 SKIP, 248 checks. | Browser/API scenarios against a running app, registered through `tests/simulation/run.mjs`; `npm run test:static` is the PR-safe syntax/project-selection smoke layer. |
...
| `backend/app/evals/` | Retrieval and answer measurements. `retrieval_eval`: span-graded qrels over a sandbox index (Recall@k, nDCG@10, MRR with bootstrap intervals), re-indexing ablations (paired randomization, Holm), budget recall per context window. `answer_eval` (live lane, spend-capped): faithfulness and context precision; each judge must agree with known labels (Cohen's kappa >= 0.60) before it counts, and is never the generator. `embedder_compare` applies the pre-registered default-embedder rule (DEC-15) to one `evaluate` report per arm. Results are evidence about Istara, never research evidence. | `cd backend && python -m app.evals.retrieval_eval evaluate`; `python -m app.evals.answer_eval --project-id <id> --generator <endpoint> --judges <endpoint> --max-usd <cap>` |
