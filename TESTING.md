...
| --- | --- | --- | --- |
| Python pytest suite | `tests/` | ~164 `test_*.py` files under `tests/` plus `pi_production/`, `pi_benchmark/`, `pi_migration/`, `pi_compat/`, `benchmarks/`, `integration/`, `petals_bridge/`; `tests/e2e_test.py` is a standalone live script. Record dated counts in `testing/TEST_HISTORY.md`, not here. | Default deterministic contract, security, governance, backend, and cross-surface coverage. |
...
| Frontend unit tests | `frontend/src/**/*.test.ts` | Vitest unit suite (counts and mutation scores live in `testing/TEST_HISTORY.md`). | Run with `npm run test:unit`. |
...
| Relay unit tests | `relay/lib/**/*.test.mjs` | Node test runner coverage for relay connection strings, heartbeat payloads, and LLM proxy request behavior. **Not run by CI** — run locally when touching `relay/`. | Run with `cd relay` then `npm test`. |
