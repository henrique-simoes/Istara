                        ...
                        for row in reader:
                            for key, value in row.items():
                                if (
                                    ...
                                    and value
                                    and _looks_useful_text(value)
                                ):
                                    nuggets.append(
                                        (value.strip(), source, [method_tag, "qualitative"])
                                    )
                except Exception as e:
                    print(f"Failed to parse {path}: {e}")
                ...
                try:
                    ...
                    for block in blocks:
                        if _looks_useful_text(block) and not block.startswith("==="):
                            nuggets.append(
                                (block.strip(), source, [method_tag, "qualitative"])
                            )
                except Exception as e:
                    print(f"Failed to parse {path}: {e}")

    if not nuggets:
        raise RuntimeError("Qualitative adapter extracted 0 nuggets; check input sources.")
