...
- id: chat_react_loop
- category: agent_loop
- bridge_status: runnable_lab_adapter_plus_production_hook
- covered: yes
...
- production_gaps:
  - The replacement worktree now has an opt-in FastAPI /chat Pi candidate hook that preserves SSE/tool contracts and registers DeepSeek at runtime; full production live coverage still needs the DeepSeek key and broader endpoint fanout. (backend/app/api/routes/chat.py:123, backend/app/core/pi_replacement.py:1)
...
- category: channels
- bridge_status: runnable_lab_adapter_plus_production_hook_with_credentials_blocked
- covered: yes
