...
def test_remote_runner_uses_one_disposable_container_per_engine():
    ...
    assert (
        "/opt/runner-venv/bin/python tests/benchmarks/long_horizon_runner.py" in inner
    )
...
def test_remote_runner_bounds_live_research_scope_without_disabling_three_model_proof():
    ...
    assert (
        'ISTARA_BENCHMARK_CODING_LIMIT="${ISTARA_BENCHMARK_CODING_LIMIT:-3}"' in outer
    )
...
def test_remote_runner_requires_a_real_backend_restart_for_combined_long_horizon():
    ...
    assert (
        "ISTARA_BENCHMARK_START_CLIENT_SANDBOXES:$ISTARA_BENCHMARK_REQUIRE_RESTART_RESUME"
        in outer
    )
    assert (
