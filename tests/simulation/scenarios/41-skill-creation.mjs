...
export async function run(ctx) {
  ...
  async function fetchProjectScoped(path, checkName, options = {}) {
    if (!projectId) {
      checks.push({
        name: checkName,
        passed: false,
        detail: "No active project id; scoped endpoint not called",
      });
      return null;
    }
    const separator = path.includes("?") ? "&" : "?";
    return fetch(
      `http://localhost:8000${path}${separator}project_id=${encodeURIComponent(projectId)}`,
      options,
    );
  }
  ...
  try {
    const res = await fetchProjectScoped(
      "/api/skills/creation-proposals/pending",
      "GET /api/skills/creation-proposals/pending responds",
  ...
  try {
    const res = await fetchProjectScoped(
      ...
      "GET /api/skills/creation-proposals/all responds",
      { headers: api._headers() },
    );
    if (res) {
      checks.push({
  ...
  try {
    const res = await fetchProjectScoped(
      ...
      "Reject nonexistent proposal returns 404",
      {
        method: "POST",
        headers: api._headers(),
        body: JSON.stringify({}),
  ...
  try {
    const res = await fetchProjectScoped(
      ...
      "Approve nonexistent proposal returns 404",
      {
        method: "POST",
        headers: api._headers(),
        body: JSON.stringify({}),
  ...
  try {
    const res = await fetchProjectScoped(
      ...
      "GET /api/skills/health/all responds",
      { headers: api._headers() },
    );
    if (res) {
      checks.push({
