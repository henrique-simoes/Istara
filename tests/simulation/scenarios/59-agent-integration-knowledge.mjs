...
export async function run(ctx) {
  ...
  try {
    ...
    const agents = await api.get(`/api/agents?${projectQuery}`);
  ...
  const integrationEndpoints = [
    { path: `/api/channels?${projectQuery}`, name: "Channels" },
    { path: `/api/surveys/integrations?${projectQuery}`, name: "Survey Integrations" },
    { path: `/api/deployments?${projectQuery}`, name: "Deployments" },
    { path: "/api/mcp/server/status", name: "MCP Server Status" },
    { path: `/api/mcp/clients?${projectQuery}`, name: "MCP Clients" },
  ];
  ...
  try {
    ...
    const analytics = await api.get(`/api/deployments/${tempDeploy.id}/analytics?${projectQuery}`);
    checks.push({
