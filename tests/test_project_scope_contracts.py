...
def test_backend_deployments_enforce_project_owned_channels_and_conversations() -> None:
    ...
    assert (
        "deployment_service.activate_deployment(\n            db,\n            deployment_id,\n            project_id=deployment.project_id,"
        in route
    )
    assert (
        "deployment_service.get_deployment_analytics(\n        db,\n        deployment_id,\n        project_id=deployment.project_id,"
        in route
    )
    assert "_get_project_deployment_conversation_or_404" in route
    assert (
        "deployment_service.get_conversation(\n        db,\n        conversation_id,\n        deployment_id=deployment.id,\n        project_id=deployment.project_id,"
        in route
    )
...
def test_research_integrity_by_id_routes_require_active_project_scope() -> None:
    ...
    assert (
        "/api/code-applications/${applicationId}/review?project_id=${encodeURIComponent(projectId)}"
        in api
    )
...
def test_findings_search_and_lists_require_active_project_scope() -> None:
    ...
    api_markers = (
        ...
        "/api/findings/nuggets?project_id=${encodeURIComponent(projectId)}",
        "evidenceChain: (findingType: string, findingId: string, projectId: string)",
        "/api/findings/${findingType}/${findingId}/evidence-chain?project_id=${encodeURIComponent(projectId)}",
        "linkEvidence: (findingType: string, findingId: string, linkId: string, linkType: string, projectId: string)",
        "/api/findings/${findingType}/${findingId}/link?project_id=${encodeURIComponent(projectId)}",
    ...
    route_markers = (
        ...
        "Nugget.project_id == scoped_project_id",
        "Fact.project_id == scoped_project_id",
        "Insight.project_id == scoped_project_id",
        "Recommendation.project_id == scoped_project_id",
        "DesignDecision.project_id == scoped_project_id",
        ...
        'project_id: str | None = Query(None, description="Active project")',
        "Global findings search requires admin access",
    )
    ...
    drilldown_markers = (
        "findingsApi.evidenceChain(type, id, projectId)",
        "findingsApi.linkEvidence(activeFinding.type, activeFinding.id, linkId, linkInfo.linkType, projectId)",
        "Research-validity gate is blocking promotion.",
    )
...
def test_task_kanban_requires_active_project_scope() -> None:
    ...
    gate_markers = (
        "const needsCodingBeforeReport =",
        "const canMarkDone =",
        "const canSendToReport =",
...
def test_llm_server_inventory_and_health_checks_require_global_admin_access() -> None:
    ...
    status_body = settings_route.split('@router.get("/settings/status")', 1)[1]
    assert "def _cached_llm_readiness() -> tuple[bool, bool]:" in settings_route
    assert all(
        marker not in status_body
        for marker in (
            "await ollama.health()",
...
def test_autoresearch_project_surfaces_require_active_project_scope() -> None:
    ...
    runner_base = read_repo("backend/app/core/autoresearch_runners/__init__.py")
    question_bank = read_repo("backend/app/core/autoresearch_runners/question_bank.py")

    assert all(
        marker in store
        for marker in (
            "autoresearch.status(projectId)",
            "autoresearch.leaderboard(projectId)",
    ...
    assert all(
        ...
        for marker in (
            ...
            "/api/autoresearch/status?project_id=",
            'p.set("project_id", params.project_id);',
            "/api/autoresearch/leaderboard?project_id=",
            "/api/autoresearch/stop?project_id=",
        )
    )
    ...
    assert all(
        ...
        for marker in (
            ...
            "AutoresearchExperiment.project_id == project_id",
            "TelemetrySpan.project_id == project_id",
        )
    )
    assert all(
        marker in runner_base
        for marker in (
...
def test_self_evolution_routes_and_engine_require_active_project_scope() -> None:
    ...
    assert (
        "Self-evolution candidate scans, auto-evolution, and promotion mutations require an explicit active project id."
        in docs
    )
...
def test_loops_views_and_api_require_active_project_scope() -> None:
    ...
    assert (
        "updateSchedule(schedule.id, { enabled: !schedule.enabled }, activeProjectId)"
        in schedules_tab
    )
...
def test_websocket_project_events_are_active_project_filtered() -> None:
    ...
    assert (
        '"agent_id"' in websocket
        and '"from_agent_id"' in websocket
        and '"to_agent_id"' in websocket
    )
...
def test_notifications_are_active_project_scoped() -> None:
    ...
    assert (
        "markRead(notification.id, activeProjectId)" in list_tab
        and "deleteNotification(notification.id, activeProjectId)" in list_tab
    )
    ...
    assert (
        "markRead: async (id, projectId) =>" in store
        and "await notificationsApi.markRead(id, projectId)" in store
    )
    assert (
        "deleteNotification: async (id, projectId) =>" in store
        and "await notificationsApi.delete(id, projectId)" in store
    )
    assert "notification.project_id !== activeProjectId" in store

    assert (
        "markRead: (id: string, projectId: string)" in api
        and "unreadCount: (projectId: string)" in api
        and "markAllRead: (projectId: string)" in api
        and "delete: (id: string, projectId: string)" in api
        and "JSON.stringify({ project_id: projectId })" in api
    )
    ...
    assert all(
        ...
        for marker in (
            ...
            "get_subject",
            "is_global_admin",
            "explicit admin-global scope",
        )
    )
    assert (
        '"agent_promotion"' in route
        and '"agent_promotion"' in types
        and "agent_promotion:" in list_tab
    )
    assert (
        '{ id: "agent_promotion", label: "Agent Promotion" }' in category_filter
        and '{ id: "agent_promotion", label: "Agent Promotion" }' in prefs_tab
    )
