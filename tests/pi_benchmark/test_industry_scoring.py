...
def test_extract_plain_json():
    assert scoring.extract_json_call('{"name": "f", "arguments": {"a": 1}}') == {"name": "f", "arguments": {"a": 1}}
...
def test_bfcl_strict_match():
    truth = [{"calculate_triangle_area": {"base": [10], "height": [5], "unit": ["units", ""]}}]
...
def test_bfcl_allowed_alternative_value():
    truth = [{"f": {"unit": ["units", ""]}}]
    assert scoring.score_bfcl(truth, '{"name": "f", "arguments": {"unit": ""}}')["score"] == 1.0
...
def test_bfcl_float_int_normalisation():
    truth = [{"f": {"n": [5]}}]
    assert scoring.score_bfcl(truth, '{"name": "f", "arguments": {"n": 5.0}}')["score"] == 1.0


def test_tau_action_match():
    assert scoring.score_tau(["book_reservation"], '{"action": "book_reservation", "rationale": "x"}')["score"] == 1.0
...
def test_score_industry_record_routes_by_expected_keys():
    assert scoring.score_industry_record({"bfcl_ground_truth": [{"f": {}}]}, '{"name":"f","arguments":{}}')["kind"] == "bfcl"
    assert scoring.score_industry_record({"tau_expected_actions": ["a"]}, '{"action":"a"}')["kind"] == "tau"
