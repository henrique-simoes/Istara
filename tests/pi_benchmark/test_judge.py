...
def test_blind_prompt_hides_engine_labels_and_swap_is_deterministic():
    layer = JudgeLayer(_config(), judge_fn=lambda prompt, arms: {"winner": "A", "score_a": 6, "score_b": 4})
...
def test_verdict_is_unblinded_to_the_real_engine():
    ...
    j = layer.judge(scenario_id="s", run_id="r", axis="output_quality",
...
def test_cache_prevents_respend():
    ...
    layer = JudgeLayer(_config(), judge_fn=judge_fn)
    first = layer.judge(scenario_id="s", run_id="r", axis="output_quality",
                        pi_output="a", legacy_output="b")
    second = layer.judge(scenario_id="s", run_id="r", axis="output_quality",
                         pi_output="a", legacy_output="b")
