...
_CORPUS_BRIEF = _REPO_ROOT / "tests" / "document_corpus" / "canonical" / "sources" / "brief" / "CR-144-brief-01.md"
...
def _case_prompt(case: dict) -> str:
    return "\n\n".join(f"[{m['role']}] {m['content']}" for m in case.get("messages", []))
...
def scenarios() -> tuple[Scenario, ...]:
    ...
    if plan:
        built.append(Scenario(
            ...
            pack=PACK, min_tier="T3",
            prompt=_case_prompt(plan),
            expected={"evals_checks": plan.get("checks", {}), "spine_phases": ["plan"]},
            tags=("deep", "dag", "plan"),
        ))
    ...
    if tool:
        built.append(Scenario(
            ...
            expected={"evals_checks": tool.get("checks", {}), "spine_phases": ["tool_selection"]},
    ...
    if excerpt:
        built.append(Scenario(
            ...
            expected={"rubric": "grounding", "spine_phases": ["grounding", "synthesis"],
    ...
    built.append(Scenario(
        ...
        expected={"markers": ["[SKILL:thematic-analysis]"],
