...
async def test_scenario5_search_memory_and_reasoning_store_are_project_scoped():
    ...
    await reasoning_bank.record_memory(
        project_id=project_a, agent_id="istara-main", source_kind="manual",
        outcome="success", title=f"Retention insight {marker}",
        content=f"Project A retention memory {marker}: onboarding drives 30-day retention.",
        confidence=0.9,
    )
    await reasoning_bank.record_memory(
        project_id=project_b, agent_id="istara-main", source_kind="manual",
        outcome="success", title="Project B secret",
    ...
    svc = faux_service(
        [tool_call("search_memory", {"query": "retention"}), final_text("Searched project memory.")],
        sup,
    )
    ...
    try:
        async for _ in svc.run_chat_turn(
            ...
            history=[], user_text="What do we know about retention?", tool_executor=scoped_exec,
    ...
    a_hits = await reasoning_bank.retrieve(project_id=project_a, query="retention", include_global=False)
    ...
    b_hits = await reasoning_bank.retrieve(project_id=project_b, query="retention", include_global=False)
    b_blob = " ".join(str(h) for h in b_hits)
    assert "Project A retention memory" not in b_blob  # cross-project content never leaks
