...
@pytest.mark.asyncio
async def test_channel_reply_rejects_nonsuccess_terminal_with_partial_text(monkeypatch, status):
    """A channel reply is not built from an error/aborted turn's partial text."""
    monkeypatch.setattr(seams, "_service", _StubChannelService(_terminal_result(status)))
...
@pytest.mark.parametrize("status", ["error", "aborted"])
@pytest.mark.asyncio
async def test_delegation_rejects_nonsuccess_terminal_with_partial_text(monkeypatch, status):
    """A delegation result from an error/aborted turn is dropped, not returned."""
    monkeypatch.setattr(seams, "_service", _StubDelegationService(_terminal_result(status)))
...
async def test_channel_fails_closed_on_real_error_after_partial_output(monkeypatch):
    ...
    async with async_session() as db:
        ...
        instance = await channel_service.create_channel_instance(
            db, platform="pi_local", name="Pi Local FC", config={"enabled": True}, project_id=project_id
        )
    ...
    monkeypatch.setattr(
        seams,
        "_service",
        faux_service([error_after_partial("Partial channel reply before failure.")], sup),
    )
    try:
...
async def test_delegation_fails_closed_on_real_error_after_partial_output(monkeypatch):
    ...
    monkeypatch.setattr(
        seams,
        "_service",
        faux_service([error_after_partial("Partial delegation result before failure.")], sup),
    )
...
@requires_node
@pytest.mark.asyncio
async def test_autoresearch_fails_closed_on_real_error_after_partial_output(monkeypatch):
