...
def test_feature_docs_manifest_matches_inventory() -> None:
    inventory = json.loads(
        (FEATURE_DOCS / "inventory.json").read_text(encoding="utf-8")
    )
    ...
    for source in (FEATURE_DOCS / "content").rglob("*.md"):
        ...
        for raw_target in re.findall(r"\[[^\]]+\]\(([^)]+\.md(?:#[^)]+)?)\)", text):
            ...
            assert resolved.exists(), (
                f"{source.relative_to(ROOT)} has broken markdown link {raw_target}"
            )
...
def test_feature_docs_site_shell_has_accessible_landmarks_and_theme_support() -> None:
    ...
    feature = (SITE_ROOT / "features/chat/overview/researcher.html").read_text(
        encoding="utf-8"
    )
    ...
    assert "navigator.clipboard.writeText" in (SITE_ROOT / "assets/site.js").read_text(
        encoding="utf-8"
    )
...
def test_feature_docs_github_pages_workflow_builds_and_deploys_site() -> None:
    ...
    assert (
        'python -m pip install pytest pytest-asyncio "sqlalchemy[asyncio]" aiosqlite pydantic-settings'
        in workflow
    )
