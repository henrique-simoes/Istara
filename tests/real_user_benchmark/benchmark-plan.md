...
Storage and shutdown are part of run hygiene. The remote Docker operator supplies passive daemon/storage evidence; the benchmark does not install packages, autostart Colima, stop Colima, or mutate host VM lifecycle. The future three-model Docker topology should reserve at least 12GB for the two llama.cpp donors, remove benchmark-owned relay/model containers at the end, and record cleanup evidence.
