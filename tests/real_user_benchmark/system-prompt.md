...
- The default first donor is Gemma (`google/gemma-4-e4b`). A fully containerized three-model benchmark must use `ISTARA_BENCHMARK_DONOR_TOPOLOGY=macstudio-colima-qwen-gemma` through `scripts/runner/docker-run.sh`; the wrapper provisions the Compose-managed Gemma donor plus the already-downloaded Qwen3.5 4B Q4 and Gemma 4 E2B Q4 llama.cpp donors, attaches relay/preflight clients to the project network, and requires the Compose Gemma GGUF to exist under the read-only model root. The direct `probe:deep:three-model` invocation is still a fail-closed diagnostic and must not be used as live evidence. Missing local GGUF files or endpoints are benchmark blockers, not something to fake or auto-download.
