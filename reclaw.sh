#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────
# reclaw.sh — Start or stop ReClaw backend + frontend
#
# Usage:
#   ./reclaw.sh          Start both (or stop if already running)
#   ./reclaw.sh start    Start backend + frontend
#   ./reclaw.sh stop     Stop backend + frontend
#   ./reclaw.sh status   Show what's running
#   ./reclaw.sh restart  Stop then start
#   ./reclaw.sh logs     Tail both log files
# ─────────────────────────────────────────────────────────

set -euo pipefail

ROOT="$(cd "$(dirname "$0")" && pwd)"
BACKEND_PID_FILE="$ROOT/.reclaw-backend.pid"
FRONTEND_PID_FILE="$ROOT/.reclaw-frontend.pid"
BACKEND_LOG="$ROOT/.reclaw-backend.log"
FRONTEND_LOG="$ROOT/.reclaw-frontend.log"
...
_start() {
    echo ""
    echo -e "${CYAN}═══ Starting ReClaw ═══${NC}"
    echo ""
    ...
    echo ""
    echo -e "  ${GREEN}ReClaw is up.${NC} Open ${CYAN}http://localhost:$FRONTEND_PORT${NC}"
    echo ""
}

_stop() {
    echo ""
    echo -e "${CYAN}═══ Stopping ReClaw ═══${NC}"
    echo ""
...
_logs() {
    echo -e "${CYAN}═══ ReClaw Logs (Ctrl+C to exit) ═══${NC}"
    echo ""
    tail -f "$BACKEND_LOG" "$FRONTEND_LOG" 2>/dev/null || echo "No log files found. Start ReClaw first."
}
