...
Istara supports the Model Context Protocol (MCP) as both a **server** (exposing capabilities to external agents) and a **client** (connecting to external MCP servers).

## SECURITY WARNING

The MCP Server allows external AI agents to access your local research data. This breaks Istara's local-only boundary. It is:
...
When enabled, exposes Istara capabilities at `http://localhost:{port}/mcp`.
...
Connect external MCP servers to augment Istara's capabilities.
