...
- **Task:** `PI-BENCH-ROLE-CORRECTION-20260722-PLAN-A` (consensus architect slot A,
  pipeline `PI-BENCH-ROLE-CORRECTION-20260722`)
- **Spec:** CF-SPEC-8 · **Lifecycle:** `docs/build-stream/2026-07-22-pi-benchmark.md`
- **Authored:** 2026-07-22, against branch `Review_pi_test` (post-L-21 state)
...
The lifecycle file contains two mutually contradictory role assignments, both verified
by direct grep on 2026-07-22:
...
| Region | Lines | Wrong text |
|---|---|---|
| Decision log DEC-5 | 489 | "Route every live evaluation **and judge** call through DeepSeek `deepseek-v4-pro`" |
| Decision log DEC-6 | 499–500 | judging cast "may use Claude, Codex, Kimi, or another configured judging harness" — leaves the judge open instead of naming Kimi as the intended judge harness/model |
...
at lines 30–31 and 76–77, judge-must-not-rerun-DUT at 79–82) and the full evaluation
matrix — it needs only a consistency audit and one small addition (explicit "MoA
route/downgrade evidence" in the pack list, and an explicit pointer to the role-correction
brief as the authoritative role statement).
...
3. **Work-order (`pi-benchmark-deepseek-moa-execution.md`) — audit + tighten.** Verify
   ...
   to the evaluation-matrix pack list; add one line naming
   `docs/build-stream/conductor-instructions/pi-benchmark-role-correction.md` as the
   authoritative role statement for every future Conductor worker; state that judging
   does not consume the evaluation ledger (already implied at line 38 — make it part of
   the role list).
...
acceptance):** canonical 15-scenario contract coverage, feature breadth, Research Spine
lifecycle, A2A collaboration, prompt/injection probes, usage/cost accounting, and MoA
route/downgrade evidence. (The lifecycle's wave contract line 43–44 lists
canonical/breadth/depth/feature/spine/A2A/probe; add usage/cost accounting and MoA
route/downgrade evidence so both documents enumerate the same seven packs.)
...
| RC-3 | Extend the wave-contract pack list with usage/cost accounting + MoA route/downgrade evidence (seven packs total) | same file, lines ~43–44 | — | S |
| RC-4 | Work-order audit + tighten: identical five-role list, seven-pack list, pointer to the role-correction brief, judge-does-not-consume-ledger in the role list | `docs/build-stream/conductor-instructions/pi-benchmark-deepseek-moa-execution.md` | — | S |
...
| Shared-worktree race with in-flight F-5/F-6/F-7 fixers (they edit tests/, lifecycle ledger) | Medium | Acquire `repo_lock.completion_lock` for the read-append-commit critical section; commit only the two doc paths via `repo_lock.commit_paths`; never `git add -A`. |
