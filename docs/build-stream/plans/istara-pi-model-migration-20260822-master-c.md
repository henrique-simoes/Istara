# MECE Master Architecture Plan — Istara Pi Model-Management Migration, 2026-08-22 run

**Task:** `ISTARA-PI-MODEL-MIGRATION-20260822-MASTER-C`
...
**Planning phase:** `synthesize` — architect consensus master candidate. No implementation, no lifecycle-plan edits, no vote in this phase.
**Synthesis provenance:** this candidate reconciles carried drafts A/B/C and the immutable repaired architect snapshots. A contributes identity, secret custody, Research Spine, and test ownership; B contributes migration states, rollback, task decomposition, and QA; C contributes current-tree boundaries, ratchets, testing lineage, and owner gates. Historical multivac material is superseded by the requested managed VPS/Dokploy profile and vps skill.
...
## 0. Synthesis coverage matrix

| Master area | A | B | C |
|---|---|---|---|
| Boundary and invariants | identity, research validity, tests | legacy consumer inventory | current-tree anchors, ratchet |
| Architecture | model/endpoint/credential split | adapter/projection details | Pi authority, fail-closed semantics |
| Migration | secret-safe mapping | states, snapshots, rollback | owner gates, testing lineage |
| Waves and proof | per-wave acceptance | task graph, QA order | current commands, inherited debt |
| Operations | security/privacy | disposable QA | strict Dokploy/VPS isolation |
...
This is the synthesized MECE master candidate for the owner approval gate. It is
complete enough to generate implementation tasks, but it authorizes no source
implementation, lifecycle-plan edit, Docker/VPS start, live provider call, model
load, merge, push, or PR. The conductor must freeze this plan, record owner
approval, and only then release wave tasks. All later waves remain on the testing
lineage and must preserve per-wave gates, command evidence, security benchmark
triggers, feature-doc generation, and explicit rollback criteria.
