# Independent S1 Draft (Slot B) — Istara Pi Model-Management Migration, 2026-08-22 run

**Task:** `ISTARA-PI-MODEL-MIGRATION-20260822-PLAN-B` (CF task id 2)
**Role:** `istara-pi-model-management-20260822-architect-b`
...
The carried-forward 2026-08-18 drafts (A/B/C) were inputs; where they disagree with
the current tree, this plan extends/corrects rather than discards.
...
| V12 | `vpsctl.py` (vps skill) exposes exactly `preflight, inventory, audit-verify, audit-anchor, verify-isolation, verify-exposure, ssh`; strict profile = **one service**, project-local `internal: true` bridge, one published port, no proxy/Domain/external network/socket/privileged/host namespace, read_only + cap_drop ALL | vps SKILL.md + `--help` |
| V13 | Test anchors verified present: `test_pi_runtime_endpoints`, `test_model_provider_contract`, `test_field_encryption`, `test_llm_servers`, `test_settings_agentic_pi_endpoints`, `test_transcription`, `test_research_validity_contract`, `test_synthetic_provisional_boundary`, `test_feature_docs`, `test_feature_obligations`, `test_qa_reset_seed`, `test_security_benchmark`, `pi_production/test_{endpoint_secrets,runtime_hardening,w1_agentic_contract,w1_dispatcher_authority,w1_usage_ledger,w1_realpath_accounting,w6_engine_selection,w8_embeddings_gateway,w8_ux_parity,engine_http_provider,seams_fail_closed,same_model_donor_isolation,w3_research_spine}`, `petals_bridge/test_petals_bridge`, `pi_migration/__init__`, `compute_cases/{config,routing,retries,status_contracts}`, `pi_benchmark/{budget_ledger,test_b1_contract,test_live_driver,test_runner}`, `test_project_scope_contracts`, `test_compute_registry_hardening` | file inventory |
| V14 | **Not yet existing** (obligations to create in later waves, never claimed as green today): `tests/pi_production/test_research_spine_donor_routing.py`, `tests/pi_migration/test_model_management_migration.py`, `tests/pi_migration/test_model_management_rollback.py` | file inventory |

### Corrections this draft makes to the carried 2026-08-18 drafts
...
- **C5 — VPS single-workload constraint is load-bearing.** The full 8-service
  ...
  the owner's written approval with the minimum connectivity graph (§9.4).
...
- **Design outputs:** embedding policy keyed by canonical model identity; selector
  UX contract (`frontend/src/lib/modelCatalog.ts`, `ProjectSettingsView.tsx`,
  scenario `79-engine-selector.mjs`); summary provenance schema.
- **Exact verification:**
  ```bash
...
- **Exact verification:**
  ...
  uv run --project backend python -m pytest tests/test_feature_docs.py tests/test_feature_obligations.py \
    tests/test_qa_stack_contract.py tests/test_qa_reset_seed.py tests/test_synthetic_provisional_boundary.py -q
  ...
  python scripts/security_benchmark.py --fail-on-threshold
  # VPS (see §9; every remote command via vpsctl.py ssh):
  python3 scripts/vpsctl.py preflight
  python3 scripts/vpsctl.py inventory
  python3 scripts/vpsctl.py verify-isolation && python3 scripts/vpsctl.py verify-exposure
  python3 scripts/vpsctl.py audit-verify && python3 scripts/vpsctl.py audit-anchor
  ```
...
|---|---|---|
| `foundation` | PLAN-A/B/C + synthesis/vote + owner gate (planning-only) | old CF-797/798/802/803 (validate/contract/inspect obligations folded into planning + later waves) |
...
1. **Preflight:** `python3 scripts/vpsctl.py preflight` — verify keychain-backed SSH
   identity (`id_ed25519_capi`, `IdentitiesOnly`, strict host-key checking), helper
   integrity, audit DB health. Never print key material.
2. **Inventory (read-only evidence):** `vpsctl.py inventory` — observed Compose
   projects/containers and current public exposure. Existing workloads are
...
6. **Verify:** `vpsctl.py verify-isolation` (one attached network, one endpoint,
   `Internal: true`, no default route, no host/other-container reachability) and
   `vpsctl.py verify-exposure` with the documented approved port set (IPv4+IPv6).
7. **Firewall evidence:** check firewall before/after and the `DOCKER-USER` chain
   ...
   workload is known.
8. **Audit:** `vpsctl.py audit-verify` then `vpsctl.py audit-anchor` (signs the
   chain head with the SSH identity). Report verified port set, image identity,
   isolation proof, firewall result, rollback state, and audit event ids — never
...
audit database are untouched. Record cleanup commands through `vpsctl.py ssh`.
...
|---|---|---|
| Verified boundary inventory | §2 | C §2 (primary), A §2 (corrections C1–C6) |
...
| Wave plan + verification commands | §7 | C §5 (command sets, environment note), A §8 (acceptance phrasing), B §7 (wave command lists) — all re-verified against this tree (V13/V14) |
| VPS acceptance (replaces multivac) | §9 | vps SKILL.md + `references/dokploy-and-isolation.md` (authoritative); carried drafts' multivac sections **superseded** (C1) |
...
This is an independent draft only (slot B). The next stage — architect consensus —
compares A/B/C, reconciles the corrections in §2 (especially C1 multivac→VPS,
C2 CF-SPEC-1 wave mapping, C5 single-service VPS reality check), and freezes one
