# Independent S1 Draft — Pi Model-Management Migration (resumed 2026-08-22)

**Task:** `ISTARA-PI-MODEL-MIGRATION-20260822-PLAN-A`
...
compass-forge gate before --task ISTARA-PI-MODEL-MIGRATION-20260822-PLAN-A --summary  # fail, 30 inherited, 0 new
...
**VPS commands (owner-authorized, all through `vpsctl.py`, never raw SSH):**

```bash
python3 scripts/vpsctl.py preflight
python3 scripts/vpsctl.py inventory
python3 scripts/vpsctl.py deploy --compose <strict-template> --project istara-pi-model-migration-<run>   # one target
python3 scripts/vpsctl.py verify-isolation --port <approved-port>
python3 scripts/vpsctl.py verify-exposure --ports <approved-port-set>
python3 scripts/vpsctl.py audit-anchor
