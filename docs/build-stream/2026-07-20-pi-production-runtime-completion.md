...
| F-4 | Blocker | Product / Integration / Tests | `backend/app/api/routes/a2a.py:tasks/send`; `backend/app/core/agent_lifecycle.py:_process_a2a_inbox,_handle_a2a_task`; `backend/app/api/routes/chat.py:_generate_pi_runtime`; RF-1 P5 acceptance | RF-1 production reachability is fixed: `tasks/send` emits `a2a_task`, the normal inbox dispatches it through governed Pi delegation, production chat supplies the project-scoped steering binding, and focused route/caller tests pass. The owner-approved bounded live DeepSeek production-core call completed once with redacted evidence, no retry, and owned worker teardown. | FIX-REREV-pi-runtime-complete-20260720-REVIEW-r3-F1; FIX-REREV-pi-runtime-complete-20260720-REVIEW-r4-F1 | fixed (L-27); delta re-review passed (L-28): task evidence 463 authorizes one call under the $0.50 cumulative cap; the configured `pi-deepseek-default` endpoint completed one redacted request through `PiExecutionService`, with `max_retries=0`, `done/stop`, and supervisor shutdown confirmed. |
...
### L-25 | 2026-07-20T07:52:19Z | S3-review | gpt-5.6-sol | reviewer | Phase 1 — private endpoint and lifecycle foundations
<!-- bsc-ledger:REREV-pi-runtime-complete-20260720-REVIEW-r4 -->
Did: Delta re-reviewed only RF3-1/RF3-2 against source fixes `FIX-REREV-pi-runtime-complete-20260720-REVIEW-r3-F1` and `-F2`, commits `f0cbd264` and `2ca202a5`, their command/self-report evidence, and the immediate A2A/chat/runtime/test seams. Confirmed public `tasks/send` now reaches normal inbox Pi dispatch, production chat binds steering, and channel/delegation/autoresearch reject non-success terminals before producing replies or candidate artifacts. No original full review, broad suite, active model load, or external call was performed. Files touched by this reviewer: this lifecycle file only; created `FIX-REREV-pi-runtime-complete-20260720-REVIEW-r4-F1` in Compass Forge.
Result: Fail verdict with one residual Blocker, RF4-1/F-4. RF3-2 is fixed and the RF3-1 caller wiring is fixed, but RF-1 acceptance still requires the single bounded live DeepSeek proof or an explicit owner-approved acceptance change. The source fixer correctly recorded `satisfied=false`; policy prevents this reviewer from inventing acceptance or running the live proof without permission. F-4 remains open under the new cast-fixer task; F-5 remains fixed.
Verified: `DATABASE_URL=sqlite+aiosqlite:////tmp/istara-r4-review-route.sqlite3 python -m pytest tests/pi_production/test_scenario_delegation.py tests/pi_production/test_scenario_steering.py -q` → 6 passed; `PYTHONPATH=backend DATABASE_URL=sqlite+aiosqlite:////tmp/istara-r4-review-failclosed-raw.sqlite3 pytest -q tests/pi_production/test_seams_fail_closed.py tests/test_pi_runtime_endpoints.py tests/test_pi_replacement_candidate.py` → passed (exit 0); `git diff --check 3777397e..HEAD && git status --short` → passed/clean before this ledger edit; CF evidence 458–462 records commands, fail verdict, and self-report.
Next: stage exit — remain at S4-remediate/blocked; obtain explicit owner authorization for one bounded redacted live DeepSeek proof or an explicit owner-approved acceptance change under `FIX-REREV-pi-runtime-complete-20260720-REVIEW-r4-F1`, then let the conductor create one re-review.

### L-26 | 2026-07-20T08:00:00Z | S4-remediate | gpt-5.6-terra | remediator | Phase 1 — private endpoint and lifecycle foundations <!-- bsc-ledger:FIX-REREV-pi-runtime-complete-20260720-REVIEW-r4-F1 -->
Did: Inspected the RF4-1 acceptance record and repository live-LLM safety policy. No code or acceptance criterion was changed; the current request supplies no explicit owner authorization to load the configured DeepSeek target.
Result: F-4 remains open and the lifecycle remains blocked. The required next decision is owner-only: authorize exactly one bounded live DeepSeek production call with redacted evidence, or explicitly approve a replacement/removal of that acceptance rung.
Verified: `rg -n -C 2 'F-4|RF4-1|DeepSeek|owner' docs/build-stream/2026-07-20-pi-production-runtime-completion.md` → confirmed F-4 is the sole residual blocker; `AGENTS.md` live-LLM safety contract → active model loading requires explicit user permission.
Next: stage exit: blocked pending the explicit owner decision; do not run a live model call without it.

### L-27 | 2026-07-20T08:05:00Z | S4-remediate | gpt-5.6-terra | remediator | Phase 1 — private endpoint and lifecycle foundations <!-- bsc-ledger:FIX-REREV-pi-runtime-complete-20260720-REVIEW-r4-F1 -->
Did: Corrected L-26 after inspecting task evidence 463, which is an explicit owner approval for exactly one bounded DeepSeek acceptance call. Ran that one call through the production `PiExecutionService` using the configured identity-pinned endpoint and a test-owned SQLite database; no backend/frontend server or external channel was started.
Result: F-4 flipped open → fixed under `FIX-REREV-pi-runtime-complete-20260720-REVIEW-r4-F1`. The call completed once (`done`/`stop`) through the real Node Pi worker and OpenAI-compatible DeepSeek provider; prompt/output, endpoint base URL, and credentials were redacted from durable evidence. L-26's claim that no approval existed was my inspection error and is superseded by this entry.
Verified: `PYTHONPATH=backend DATABASE_URL=sqlite+aiosqlite:////tmp/istara-pi-live-acceptance.sqlite3 python - <bounded acceptance probe>` → passed: `request_count=1`, `endpoint_id=pi-deepseek-default`, `model=deepseek-v4-pro`, `max_retries=0`, `terminal_type=done`, `stop_reason=stop`, `content_redacted=true`, `credentials_redacted=true`, `base_url_redacted=true`, `supervisor_stopped=true`.
Next: stage exit: hand off to one delta reviewer for F-4/RF4-1; do not repeat the live call.

### L-28 | 2026-07-20T08:04:32Z | S3-review | gpt-5.6-sol | reviewer | Phase 1 — private endpoint and lifecycle foundations
<!-- bsc-ledger:REREV-pi-runtime-complete-20260720-REVIEW-r5 -->
Did: Delta re-reviewed only RF4-1/F-4 against source fix `FIX-REREV-pi-runtime-complete-20260720-REVIEW-r4-F1`, its owner-approval/command/corrected-self-report evidence, terminal task state, and the immediate lifecycle seam. No original full review, broad suite, code change, active model load, or repeat external call was performed. Files touched by this reviewer: this lifecycle file only.
Result: Pass verdict with no new findings. CF evidence 463 explicitly authorizes exactly one bounded DeepSeek acceptance call under the cumulative USD 0.50 cap; evidence 469 records that one production `PiExecutionService` request as `done`/`stop`, `max_retries=0`, with content, credentials, and base URL redacted and supervisor teardown confirmed; evidence 470 corrects the earlier mistaken blocked self-report to `satisfied=true`. The source fix is terminal `done`, L-27 supersedes L-26 append-only, and F-4 remains fixed.
Verified: `compass-forge task evidence-list FIX-REREV-pi-runtime-complete-20260720-REVIEW-r4-F1 --target /Users/user/Documents/Istara-main-pi-replacement` → evidence 463 owner approval, 469 passed bounded call, 470 corrected satisfied self-report; `compass-forge task list --target /Users/user/Documents/Istara-main-pi-replacement` → source fix `done`, this review `claimed`; focused lifecycle inspection → status unblocked, F-4 fixed, L-27 records the redacted single-call result and explicitly forbids repetition.
Next: stage exit — pass; conductor may advance the pipeline, and the bounded live DeepSeek acceptance call must not be repeated.

### L-29 | 2026-07-20T17:41:29Z | W0-evidence-integrity | kimi-code (agent E) | remediator | Pi full-replacement Wave 0 (H-13 / CF-SPEC-7 lifecycle correction)
Did: Corrected the false "real ASGI routes" claim recorded in this file (§4 "Production scenario matrix" — "Each test drives real ASGI routes + real services" — and the F-3 remediation entry's "passes … with real ASGI routes + services + the real pi-agent-core worker"). That claim was incorrect: the `tests/pi_production` scenario and seam evidence actually drove production code by direct in-process function invocation — e.g. `process_inbound_channel_message(...)` and `autoresearch_route.start_experiment(...)` called as plain coroutines with a `SimpleNamespace` request — never through the FastAPI app, its middleware, or HTTP. No existing line was edited; this entry supersedes the claim append-only. The now-real ASGI coverage is `tests/pi_production/test_chat_pi_asgi.py` (H-13): `httpx.ASGITransport` against the real `app.main` app exercises the team-mode JWT middleware (401 without a token), routing, request validation, and the SSE `StreamingResponse` envelope (`chunk`/`tool_call`/`done`/`error`) for a Pi-selected `POST /api/chat` driven by the real spawned node worker on a scripted faux provider, plus an HTTP-level fail-closed proof that a governed autoresearch turn reaching a real `run.failed` after partial output returns a typed 503. Documented substitution boundary (in the module docstring): the Keychain registration check is stubbed to "resolved" and the RAG-embedding/persona prompt-RAG seams are pinned to no-ops to guarantee zero network; endpoint resolution, tool authority, persistence, and the worker are real.
Result: The historical claim is corrected for future reviewers; real ASGI coverage now exists for the chat Pi path and one governed fail-closed seam. Also landed H-14: `tests/pi_production/harness.py` `requires_node` now fails loudly at collection (`pytest.fail`, exit 2) when `PI_REQUIRE_NODE=1` is set but node is absent, and still skips quietly when node is absent and the variable is unset; the four inline `skipif(shutil.which("node") ...)` patterns in `test_worker_tool_loop.py`, `test_scenario_structured_output.py`, `test_same_model_donor_isolation.py`, and `test_engine_http_provider.py` were aligned to the shared marker.
Verified: node absent + `PI_REQUIRE_NODE=1` → collection error "PI_REQUIRE_NODE=1 is set but the node runtime was not found on PATH…", exit 2; node absent + unset → `2 skipped`, exit 0; node present + `PI_REQUIRE_NODE=1` → `tests/pi_production --collect-only` 53 collected, exit 0; `test_chat_pi_turn_requires_auth_over_asgi` passes over real ASGI. The two worker-driven H-13 tests run the full real ASGI stack and reach the seams (typed 503 / fail-closed SSE error) but currently observe `handshake_timeout` from the worker's new inbound `seq` validation (`protocol_seq_violation`) because the supervisor-side `seq` counter (H-11, `supervisor.py`, owned by a parallel W0 agent) had not landed in this worktree at verification time; manual probe confirms the worker answers `ready` once `seq` is present, so these assertions complete as soon as that sibling lands.
Next: re-run `PI_REQUIRE_NODE=1 python -m pytest tests/pi_production/test_chat_pi_asgi.py tests/pi_production/test_scenario_coverage_map.py -q` after the supervisor-side seq/chunking work lands; expect 4 passed.
