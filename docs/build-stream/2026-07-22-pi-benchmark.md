...
Verified: custom G-R2/manifest/attestation/record/route/leak audit -> passed; `<repo-root>-pi-replacement/backend/.venv/bin/python tests/pi_benchmark/verify_budget_ledger.py --ledger tests/pi_benchmark/.results/runs/retake/budget-ledger.json --cap-usd 1.00` -> `[ok]` (25 rows, provider `deepseek`); `PYTHONPATH=$PWD/backend <repo-root>-pi-replacement/backend/.venv/bin/python -m pytest tests/pi_benchmark/ -q` -> 181 passed; focused resume-dedup test -> 1 passed; `git diff --check` -> passed.
...
Verified: `PYTHONPATH=$PWD/backend <repo-root>-pi-replacement/backend/.venv/bin/python -m pytest tests/pi_benchmark/test_live_driver.py tests/pi_benchmark/test_runner.py -q` -> 47 passed (the passing tests encode the bypass); contract/source `rg` audit -> Plan C requires both DUT arms through `AgenticDispatcher.ensemble`, while `live_driver.py:462-470` returns through the direct provider and `test_live_driver.py:220` forbids the agentic path; `<repo-root>-pi-replacement/backend/.venv/bin/python -m compileall -q tests/pi_benchmark/live_driver.py tests/pi_benchmark/test_live_driver.py` -> passed; `git diff --check` -> passed.
