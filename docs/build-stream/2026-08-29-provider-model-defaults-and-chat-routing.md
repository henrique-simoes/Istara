# Provider Model Defaults & Chat Routing — Build Stream lifecycle

```yaml
item: provider-model-defaults-and-chat-routing
branch: testing
cf: { spec: CF-SPEC-7, task: CF-61 }
phase: "Phase 2 — execute expanded routing and model-role contract"
stage: S2-execute
status: in-progress
blocked_on: null
...
Persist one global main-chat endpoint, make provider credential requirements
and readiness truthful, route Chat through either agentic core, and preserve
explicit session overrides. Separately persist and order distinct connected
model identities for the Research Spine ensemble. Embedding selection remains
immutable; provider secrets remain in server custody; deterministic QA does not
count as live-provider acceptance.

## Ledger

### L-1 | 2026-08-29 | S0-frame/S1-plan | codex | planner

Did: Reproduced the reported QA behavior in the local browser and traced the
settings model label to the legacy local transport. Confirmed the configured
provider endpoint was visible but disabled when the Istara core was selected;
the legacy provider seam also dropped the session endpoint before dispatch.
Created and clarified CF-SPEC-7, generated tasks, and recorded the gate-before
baseline.

Verified: Settings/chat UI reproduction; Compass Forge status/next/agent-brief,
impact/why/context, spec plan/tasks, and gate-before baseline record 432.

### L-2 | 2026-08-29 | S2-execute | codex | implementer

Did: Added the persisted `PI_DEFAULT_ENDPOINT_ID` setting, manager-owned
default resolution, add/switch/delete API lifecycle, `default_endpoint_id` and
`default_model` metadata, endpoint forwarding through both legacy tool loops,
and the Settings/Chat controls. Added red regression tests first, then made
them green.

Verified: Three targeted tests passed for default lifecycle, endpoint forwarding,
and manager resolution; `git diff --check` passed. No live Gemini request or
credential output was used.

Next: Run the full relevant deterministic suites, frontend verification,
feature-doc generation, security benchmark, gate-after/evidence, then rebuild
the isolated Mac Studio QA stack from this branch.

### L-3 | 2026-08-29 | S3-review/S4-ship-learn | codex | verifier

Did: Ran the targeted and broader backend suites, frontend unit/build/lint,
feature-doc generation/check, security benchmark, and Compass Forge gate-after.
Rebuilt the isolated `istara-qa-testing-20260829` Mac Studio project from the
patched `testing` checkout using its existing loopback-only runtime override.

Verified: 58 targeted backend tests, 44 broader chat/settings tests, 16
frontend unit tests, TypeScript production build, ESLint, feature-doc check,
and security benchmark 28/28 passed. QA health returned `healthy`; the UI
showed first-provider auto-default (`gemini-2.5-flash`), global switch to
`gemini-2.5-pro`, Chat inheritance of that default, and a per-chat override
back to Flash. No live Gemini request or API key was used. Gate-after record
433 found no new forbidden dependencies/import cycles/missing paths but still
reports inherited complexity, route/type drift, and secret-flow debt.

Remaining: CF-SPEC-7 verifier tasks CF-61, CF-62, and CF-64–CF-69 remain open; implementation task CF-63 is done.

### L-4 | 2026-08-29 | S0-frame/S1-plan | codex | planner

Did: Reopened the lifecycle after owner testing disproved the prior handoff.
Browser reproduction showed `Failed to fetch` in Chat and a delayed second-
provider credential step. Bounded Docker logs and an independent preflight
probe established the transport cause: `OPTIONS /api/chat` returned 400
`Disallowed CORS headers` because the frontend sends
`x-istara-agent-engine`, which the backend CORS allowlist omitted. The Settings
UI also labeled API-key endpoints as having a configured secret based only on
auth-method metadata, even when no secret was entered. Refreshed Compass Forge
run 11, swept routing/credential/research-validity dependencies, revised and
re-planned CF-SPEC-7 for the expanded contract, opened CF-61 work, and recorded
gate-before baseline 435.

Verified: The allowed origin itself was accepted, the exact engine header was
the rejected preflight component, and the chat handler/provider stub received
no POST. A second provider's key field currently appears only after model
selection; no key was entered or disclosed. The refreshed context remained
incomplete (86 budget omissions), so targeted graph impact/why, textual search,
and broader tests are required before closure.

Next: Add one red regression at a time for exact CORS handling, credential
readiness/custody, and ordered distinct Research Spine endpoint roles; then
implement, update living feature docs, rebuild the isolated QA stack, and run
both-core browser verification.

### L-5 | 2026-08-30T20:53:05Z | S2-execute | codex | implementer/verifier

Owner continuity directive: append every material reproduction, edit, proof,
failure, and remaining action to the durable ledger before continuing so this
campaign can be resumed without relying on chat context. This entry captures
the pre-directive work; future material changes append new entries here and in
the parent QA campaign ledger.

Branch/worktree truth: `testing` at `8c4e54f1` (initially equal to
`origin/testing`), with the pre-existing provider/routing work preserved plus
the changes below. No commit, push, merge, PR, promotion, or `origin/main`
mutation has occurred. The worktree remains intentionally dirty; protected
`LLMs/` and `Model_Finetuning/` were untouched.

Reproduced in the loopback-only Mac Studio QA stack
`istara-qa-testing-20260829`:

1. Realtime was permanently reconnecting. The QA image embedded
   `NEXT_PUBLIC_WS_URL=ws://localhost:8000/ws`, while the client appends `/ws`,
   so every browser attempt hit `/ws/ws` and received 403.
2. A Pi SSE failure emitted `detail`/`error` but no `message`; the frontend
   reads `event.message`, so a failed turn disappeared without an assistant
   response.
3. The chat catalog treated the built-in DeepSeek endpoint as enabled/default
   even when its Keychain credential was absent. The main Pi route also called
   `ensure_pi_deepseek_registered()` for every Pi turn, so a valid selected
   provider could be blocked by an unrelated built-in endpoint.
4. Browser observations still requiring separate disposition: Passkeys and
   Active Sessions report `Invalid or revoked authentication session.` after a
   valid login; the update card exposes a raw offline DNS exception; System
   Status labels the deterministic contract stub as connected/chat-ready; the
   project sidebar badge says `Engine: Istara` while the chat toolbar resolves
   `Core: Pi`; 127.0.0.1 login is rejected while localhost succeeds.

Implemented with red-green tests:

- `docker-compose.qa.yml`: both UI images now receive the WebSocket base
  `ws://localhost:8000`; `/ws` is appended only by the client.
- `tests/test_qa_stack_contract.py`: added the base-URL contract. Red: 1
  failure; green: focused QA contract suite 6 passed. Render check passed.
- `backend/app/api/routes/chat.py`: all Pi error event shapes now include a
  user-visible `message`; internal reason remains in `detail`. The main Pi
  preflight now resolves the exact project/model/endpoint selection via
  `PiModelManager`, off the event loop, and never contacts the provider.
  Missing credentials map to actionable Settings guidance.
- Chat catalog projection now annotates only non-secret
  `credential_status=ready|missing|unavailable` and an internal reason code;
  an unavailable preferred endpoint cannot become the returned default.
- `frontend/src/lib/types.ts`, `modelCatalog.ts`,
  `modelCatalog.test.ts`, and `ChatModelControls.tsx`: readiness is fail-closed;
  only server-resolved `ready` endpoints are selectable or auto-selected.
- `tests/test_pi_replacement_candidate.py`: added selected-endpoint preflight,
  unavailable-default, and user-visible-error regressions. Red: 3 failures;
  green: 22 Pi candidate tests. Combined Chat/Pi suite: 37 passed.

Verification recorded before this entry: frontend model-catalog unit file 9
passed; frontend ESLint and production Next.js build passed; `git diff --check`
passed before the latest additions. Targeted Ruff still reports 36 inherited
issues in the large `chat.py` file (unused imports/import order/line length and
other pre-existing debt); the command stopped before frontend verification, so
frontend lint/build were rerun separately and passed. Do not misclassify that
Ruff debt as a clean gate or as introduced by the new helper without a
baseline comparison.

Live rebuild: synced only the named patched files to the isolated remote
checkout, rebuilt `qa-backend` and `qa-frontend`, recreated them under project
`istara-qa-testing-20260829`, and retained the loopback-only proxies/provider
stub. Backend reached `healthy`; frontend health/reload and post-recreate user
state still need reconciliation. Because the QA database is disposable
container `/tmp` state, browser state from before recreation is not valid proof
until a reload/re-onboarding pass. No live external model request or credential
was used or exposed.

Next, in order: (1) reload/re-onboard and capture browser proof for WebSocket,
catalog readiness, and actionable Pi failure; (2) update/regenerate living
feature docs; (3) continue the master scenarios through Documents, task
creation, Kanban state transitions, steering, loops, long horizon, Settings,
and every More/menu surface; (4) diagnose/fix each reproduced defect with its
own red-green entry; (5) run security benchmark, broad deterministic suites,
feature-doc check, CF gate-after/evidence, independent review, and release
reconciliation. Live-model quality remains explicitly unaccepted until one
owner-authorized configured target is available.

### L-6 | 2026-08-31T00:03:10Z | S2-execute | codex | implementer/verifier

Completed the post-rebuild user path in the loopback-only QA UI: created
`Testing Branch User Simulation`, skipped host-folder linking, saved synthetic
company/research context, uploaded the tracked
`tests/fixtures/interview_p1_sarah.txt` fixture, and completed onboarding. The
hidden file input did not emit a chooser through the browser harness; retrying
the same user action through the visible upload button succeeded and displayed
the filename, so this is recorded as harness targeting rather than an Istara
upload failure. The optional post-onboarding tour was skipped. No personal file
or external destination was involved.

The first live verification disproved part of L-5: an existing chat session
still showed DeepSeek V4 Pro as enabled/selected. The backend projection was
correct (`pi-deepseek-default` = `missing_keychain_secret`, no ready default),
but `ChatModelControls` unconditionally reinserted a persisted session override
as an enabled `Current session model`, then preferred it even when its exact
endpoint was unavailable. The QA backend container has no DeepSeek/Pi/cloud-key
environment variable; only credential names were inspected, never values.

Implemented a second red-green correction:

- Added `isPiSessionOverrideReady` to require an exact server-resolved,
  credential-ready endpoint/model pair before preserving a Pi session
  override.
- Guarded both the fallback choice insertion and selected-choice precedence in
  `ChatModelControls`; unavailable/stale overrides now fall through to a ready
  default or to no model.
- Added a unit regression covering missing, ready, and mismatched exact
  endpoints. Red: one failed test (`isPiSessionOverrideReady` absent). Green:
  model-catalog unit file 10 passed; frontend ESLint passed.
- Compass Forge impact/why/test-impact/suggest-tests were rerun for the exact
  stale-session defect. The graph classifies the surface as full blast radius
  and recommends the chat-session/model persistence simulations plus broad
  architecture/security checks at closure.

Deployment/proof correction: the first rebuild targeted `qa-frontend`, which
is internal and is not the loopback-published UI. Browser evidence exposed the
mistake. Rebuilding/recreating the actual `qa-ui` service from the same patched
checkout published image manifest
`sha256:abec91a3e2bcd8d6a65093bac060e70bb5423460fb29a32fe60b54003cbc971a`;
the image's Next production build completed successfully and the loopback page
returned HTTP 200.

Fresh browser acceptance after reloading the published image:

1. Project navigation resolved `Engine: Pi`; Chat resolved `Core: Pi`; realtime
   footer remained `Live updates connected` with no browser errors.
2. The toolbar displayed `Choose a model`, not DeepSeek. Searching the catalog
   for `deepseek-v4-pro` showed the DeepSeek and alias rows disabled with
   `Configure in Settings`.
3. A real chat submission through the persisted stale session never contacted
   a provider and produced the visible assistant message: Pi cannot start
   because the selected model has no available credential and the user should
   add/reconnect it under `Settings > Providers, models, and sign-in`.

New/freshly reconfirmed findings still open, deliberately not folded into this
repair: switching the Settings core radio updates the selector immediately but
the project sidebar badge remains stale until a page reload; Passkeys and
Active Sessions both show `Not authenticated` immediately after a valid admin
login; Software Updates exposes a raw DNS exception while offline; System
Status calls the deterministic contract stub `Connected` and advertises its
model as the default. These require separate causal traces and tests.

Next, in order: regenerate/check living chat/settings feature docs and record
the output; diagnose and red-green the Passkeys/Active Sessions session failure;
then disposition the reactive core badge, update-status copy, and contract-stub
truthfulness before moving through Documents, Tasks/Kanban, steering, Loops,
long-horizon, and every remaining menu surface. Append the next material result
before changing surfaces. No commit, push, merge, PR, promotion, live model
load, external provider request, or `origin/main` mutation occurred.

### L-7 | 2026-08-31T00:04:47Z | S2-execute | codex | verifier

Regenerated the living feature site/manifests after the chat/model-control
changes with `python scripts/feature_docs.py --seed-missing --generate-site
--check`: seeded 0 files, generated 224 site artifacts, and passed the contract
for all 86 features. `git diff --check` also passed. Next remains the freshly
reproduced Passkeys/Active Sessions authentication failure; no new surface was
opened before recording this proof.

### L-8 | 2026-08-31T00:06:03Z | S2-execute | codex | verifier

Rechecked the historical Passkeys/Active Sessions authentication observation
on the corrected published image before editing. It does not reproduce:
Passkeys loads the empty registered-credential state, Active Sessions lists the
current password-authenticated device, `/api/auth/sessions` returns 200, and a
hard reload is clean in both the immediate and settled DOM. No authentication
fix was made because current evidence does not support one. The earlier error
is retained as a transient pre-rebuild observation, not a confirmed-current
bug. Next are the deterministic raw update error and contract-stub status
truthfulness defects still visible on every Settings load.

### L-9 | 2026-08-31T00:14:32Z | S2-execute | codex | implementer/verifier

Reproduced and repaired the two deterministic Settings truthfulness defects
before opening another menu surface. The update route returned its raw
`OSError` text, including resolver details, directly to Software Updates.
Separately, System Status treated cached transport reachability as chat
readiness and, with Pi selected, substituted the local contract transport's
`active_model` when there was no credential-ready Pi default. The Pi identity
catalog also unconditionally selected the built-in DeepSeek row as the default
without checking its passive credential state.

Red-green changes:

- Added an update transport regression that injects the exact DNS-style
  failure and requires the stable `update_check_unavailable` code, actionable
  copy, and absence of `Errno`/resolver text. Red: missing `error_code`; green:
  the route logs the internal exception and returns only safe user copy.
- Added a Pi-default regression requiring an uncredentialed built-in catalog
  identity to resolve to no default. Red: DeepSeek was returned; green:
  `_pi_default_info` accepts only passive `ready`/`stored` credentials and
  fails closed if status resolution itself fails.
- Added frontend contracts for three distinct LLM states (`disconnected`,
  `not_ready`, `ready`) and engine-authoritative model display. Red: both
  helpers were absent; green: Settings now says `Reachable, chat unavailable`
  for the deterministic transport and `Not configured` when Pi has no ready
  default, never the local stub model.

Verification: 42 focused backend Settings/update tests passed; the frontend
model-catalog file passed 12 tests; focused ESLint passed; feature-doc
generation seeded 0 files, generated 224 site artifacts, and passed all 86
features; `git diff --check` passed. The first ESLint invocation used paths
relative to the repo while npm executed from the repo root, and the first diff
check raced the feature-site generator deleting/replacing generated files;
both were rerun in the correct working directory/after generation and passed.
Living docs now record safe update errors, reachability versus chat readiness,
and credential-ready Pi-default semantics.

No provider request, model load, credential mutation, external publication,
commit, push, merge, PR, or `origin/main` change occurred. Next: rebuild the
isolated `qa-backend` and published `qa-ui` once, restore the disposable
synthetic project/session, and prove the corrected Settings card in the
browser. Append that live proof before tracing the stale reactive sidebar
engine badge or moving to Documents/Tasks/Kanban.

### L-10 | 2026-08-31T00:20:57Z | S2-execute | codex | verifier

Published L-9 to the isolated QA checkout and rebuilt both actual images. The
backend image manifest is
`sha256:7a0ecfab933c6b7dfd73c318e4aed49212154ad81bb47bdf56639b8da7d03d12`;
the published `qa-ui` manifest is
`sha256:d3809d97c617f6ddfd0f1590b4b2502ffd2ba2586b1f1ffdc0a465444b879c97`.
The first remote build command used a non-login SSH PATH and could not find
Docker; rerunning with the host's explicit `/usr/local/bin/docker` path built
both images. Forced recreation briefly returned proxy 502/UI reset while the
backend initialized; both loopback endpoints then recovered (`/api/health`
healthy, UI HTTP 200). No external service was exposed.

The disposable database was recreated as expected. A synthetic admin password
reset first referenced a nonexistent `is_active` column, then wrote the role as
lowercase `admin`, which caused the first login retry to return 500 because the
SQLAlchemy enum stores `ADMIN`. The reset was corrected to the actual schema
and the synthetic admin login/onboarding succeeded. These setup-command errors
are logged as harness corrections, not Istara product defects.

Recreated-image browser evidence disproved live closure of the status fix:
onboarding correctly rendered `No local LLM server detected`, but the same
fresh Settings page rendered `LLM (Ollama): Chat ready` and
`Default Chat Model: istara-qa-contract-chat:latest`. L-9 fixed frontend state
collapse and Pi-default credential filtering, but the backend cached-readiness
source still marks the deterministic contract transport chat-ready, and the
legacy display helper still names an active model even when chat is not
actually available. This remains open; no false live-acceptance claim is made.

The fresh database also reconfirmed a separate user-visible defect: Passkeys
and Active Sessions both show `Not authenticated` immediately after a valid
admin login, while other authenticated Settings panels load and realtime is
connected. This fresh-state reproduction supersedes L-8's transient
non-reproduction and remains separate from the LLM-status repair.

Next: add failing contracts that force contract-stub `chat_ready=false` while
preserving transport reachability and suppress every default-model label when
chat is not ready; implement, rerun focused tests/docs, republish once, and
append the same-page result. Then trace the Passkeys/Sessions auth APIs before
moving to the sidebar badge or another menu.

### L-11 | 2026-08-31T00:25:21Z | S2-execute | codex | implementer/verifier

Closed the recreated-image contract-stub truthfulness gap locally with exact
red-green contracts. The first backend test draft accidentally supplied a
not-ready fake node and passed before exercising the defect; it was corrected
to supply a ready transport node, then failed on the observed false
`chat_ready=true` result. The frontend red contract likewise failed because
the legacy status helper still returned the contract model while cached chat
readiness was false.

The backend cached-readiness projection now keeps transport reachability but
forces `chat_ready=false` whenever the deterministic QA contract transport is
enabled. The frontend Settings helper now displays a legacy active model only
when the cached local transport is chat-ready; Pi remains independently
authoritative through its credential-ready `default_model`. The Settings
feature contract records both distinctions.

Verification: 43 backend Settings/update tests passed; all 12 frontend
model-catalog tests passed; focused ESLint passed; feature-doc generation
seeded 0 files, generated 224 site artifacts, and passed all 86 features;
`git diff --check` passed. No provider request, model load, remote mutation,
commit, push, merge, PR, promotion, or `origin/main` change occurred in this
landing. Next is one isolated backend/published-UI republish plus same-page
browser proof, which must be appended before beginning the separate
Passkeys/Sessions API trace.

### L-12 | 2026-08-31T00:30:48Z | S2-execute | codex | verifier

Republished L-11 into the actual isolated images and proved the corrected state
through the published browser route. The new backend manifest list is
`sha256:03ac7274e4ddc498d88dfa1bf6b832741be8a7acdc5f5e540b19d0f32c932e79`;
the new `qa-ui` manifest list is
`sha256:275d4f3ff73264c20d43d49c6231aa59f37fec9d02c93476913e8eb42bcddc42`.
Both builds completed, the backend reached healthy, and the loopback UI
restarted successfully.

An attempted database-preservation step stopped the backend before copying its
SQLite file, but the database lives on the container's `/tmp` tmpfs and was no
longer copyable once stopped. The disposable QA database therefore reset and
the synthetic admin credential was reset against the actual `ADMIN` enum
schema before browser login. This is recorded as a QA-harness lifecycle
correction, not a product-data persistence claim or an Istara defect.

Same-page browser evidence now agrees end to end: Donate Compute says no local
LLM was detected; System Status says `Reachable, chat unavailable`; Default
Chat Model says `Not configured`; the footer says `LLM connected; chat model
not ready`; the safe Software Updates copy remains visible; and realtime is
connected. The deterministic contract transport remains present only as
compatibility inventory, not a chat-ready default. No provider request or
model load occurred. The status repair is live-verified and closed. The next
surface is the separately reproduced Passkeys/Active Sessions authentication
failure on this same fresh login.

### L-13 | 2026-08-31T00:35:50Z | S2-execute | codex | implementer/verifier

Traced the fresh-login Passkeys/Active Sessions failure to a frontend bootstrap
split rather than a backend rejection. The main `LoginScreen` credential,
passkey, registration, and connection-string paths write the returned JWT to
browser storage and call `HomeClient.bootstrapAuth()`. Its successful
`fetchMe()` populated the user but did not hydrate the same JWT into Zustand.
Most Settings clients read browser storage directly and succeeded; Passkeys and
Active Sessions uniquely read `authStore.token`, failed locally with `Not
authenticated`, and therefore emitted no API request. Backend logs matched
that exact absence while other authenticated Settings requests returned 200.

Red-green: added `frontend/src/stores/authStore.test.ts`, importing the store
without a token, then placing a fresh JWT in browser storage and running
`fetchMe()` before both protected Settings actions. Red: the store token stayed
null. Green: successful `fetchMe()` now hydrates both the validated user and
the same token, after which session and passkey listing resolve.

Verification: focused regression passed 1/1; all frontend unit tests passed
(21/21 across 4 files); focused ESLint passed; production Next build and
TypeScript passed; backend auth-security tests passed 34/34; the tracked
security benchmark passed 28/28 controls at 100% (its path detector reported
no triggered backend-auth path for this frontend-only change); feature docs
seeded 0 files, generated 224 artifacts, and passed all 86 features; diff check
passed. Login, security-factor, and active-session living contracts now record
the shared token-hydration invariant. Next is a `qa-ui`-only rebuild, explicit
sign-out/fresh sign-in, and browser/API proof before moving to another surface.

### L-14 | 2026-08-31T05:54:56Z | S2-execute | codex | verifier

Published only the corrected QA UI, preserving the running backend and its
disposable database. The published `qa-ui` manifest list is
`sha256:c719b56379c9447edca24a71de304ed06c2e055463f976c4d196c30dc390afcf`;
the backend remained the L-12 image and was not rebuilt or recreated.

Live acceptance used the actual user menu and LoginScreen, not a stored-token
reload: sign out, password sign in, first-run welcome, degraded no-local-LLM
provider check, creation of the synthetic `QA User Journey 2026-08-30`
project, optional folder-link skip, project-context save, upload-step skip,
and guided-tour dismissal. The resulting Settings page showed no Passkeys
authentication error, showed `No passkeys registered yet`, and rendered the
current password session as `This device` with its Sign Out action. The same
backend log sequence records successful `POST /api/auth/login`,
`GET /api/auth/me`, `GET /api/webauthn/credentials`, and
`GET /api/auth/sessions` responses, all HTTP 200. A later reopened browser tab
repeated both protected GETs at HTTP 200. The shared token-hydration repair is
therefore live-verified and closed.

The browser reconnection found an empty tab list after app cleanup and opened
a fresh loopback tab; the authenticated project and Settings state persisted.
The first remote Compose status command omitted the contract profile and
reported the profiled provider dependency as undefined, so service inspection
used the already-running Docker container identities directly. These are QA
instrumentation corrections, not product defects. No provider request, model
load, external exposure, commit, push, merge, PR, or `origin/main` change
occurred. Next: exercise bounded synthetic document upload and governed
ingestion, log it, then continue to Tasks/Kanban.

### L-15 | 2026-08-31T06:03:40Z | S2-execute | codex | implementer/verifier

Exercised Documents as a real user with the tracked synthetic fixture
`tests/fixtures/interview_p1_sarah.txt`. Upload succeeded and produced
`Interview P1 Sarah`, status `ready`, source `User Upload`, Discover phase, and
29 source units. Preview rendered the complete 3.6 KB source plus upload-
security provenance and correctly classified it as `raw_source`,
`Reportable: No`, with the explicit evidence-unit, independent coding,
reliability/reconciliation, and Done-task requirements. Content search,
nonmatching empty state, phase/source filters, clear filters, compact/grid/list
controls, and Sync's `No new files` result all behaved correctly.

The Organize control reproduced a separate user-visible defect: when the chat
route returned an SSE `error` event because no chat-ready model was configured,
`InteractiveSuggestionBox` ignored every non-chunk event. It left a blank,
apparently idle conversation, and its Stop button also never forwarded the
created `AbortSignal` into `chatApi.send`.

Red-green coverage now lives in `frontend/src/lib/suggestionStream.test.ts`.
Red first failed because the stream consumer did not exist, then proved a
missing abort-forwarding helper. Green: the shared consumer accumulates chunks,
turns server `error` events into visible terminal failures, and the stream
factory forwards the active signal. The component shows loading even before
the first message, removes a still-empty assistant placeholder on failure,
surfaces the server guidance, and performs real request cancellation.

Verification: focused tests passed 3/3; all frontend units passed 24/24 across
5 files; focused ESLint passed; the production Next build and TypeScript
passed; living feature docs generated 224 artifacts and passed all 86
features; `git diff --check` passed. Document-suggestion architecture and
researcher contracts now record provisional-only output, visible failure,
and cancellation behavior. No live provider/model was contacted. Next is a
`qa-ui`-only republish and browser proof before Tasks/Kanban.

### L-16 | 2026-08-31T06:07:27Z | S2-execute | codex | verifier

Published only the Documents suggestion-stream repair to the isolated QA UI.
The running `qa-ui` image is
`sha256:06fcad98948351695669b82b1cc7cf64af5905e385cc8ca17f54aecb04645cd0`;
the backend remained running on
`sha256:03ac7274e4ddc498d88dfa1bf6b832741be8a7acdc5f5e540b19d0f32c932e79`
and was neither rebuilt nor recreated.

The same authenticated project and uploaded `Interview P1 Sarah` document
survived the UI-only publication. In the loopback browser, Organize now opens
an `Organization Suggestions` region whose conversation visibly says that
chat is unavailable because the deployment uses the QA contract stub, then
instructs the user to configure a live provider or configured Pi endpoint.
The former blank assistant panel, permanent ambiguity, and cosmetic Stop path
are gone; the follow-up control is correctly disabled after the terminal
failure. A rendered screenshot and accessibility snapshot agree on that state.
Backend evidence records `POST /api/sessions` as 201 and `POST /api/chat` as
200 for the SSE error contract, plus the explicit fail-closed rejection log.

No live provider request, model load, backend/database restart, external
exposure, commit, push, merge, PR, or `origin/main` change occurred. The
Documents Organize defect is live-verified and closed. Next is Tasks/Kanban
user simulation, with its result appended before another menu surface.

### L-17 | 2026-08-31T06:11:31Z | S2-execute | codex | investigator

Started the bounded Tasks/Kanban journey and created
`Open-code Interview P1 Sarah into provisional evidence atoms` in Backlog.
The task editor accepted and retained a Thematic Analysis selection, synthetic
QA label, explicit provisional/reliability/reconciliation instructions,
project context, and the uploaded interview as an input document. Before any
Research Spine promotion, the editor truthfully showed zero coding runs,
accepted codes, reconciliations, graph edges, and report links.

This walkthrough reproduced a new realtime truth defect. The autonomous worker
completed its attempt with `Verification failed: No candidate evidence
(nuggets or facts) proposed.` and correctly left the task In Progress with a
failed-review note, but `ToastNotification` treats every 100% `task_progress`
event as success. It displayed the contradictory green title `Task Complete`
above that failure message. The backend separately emitted agent status
`warning`, which the toast component also ignores. Backend logs confirm the
configured provider credential was absent, transcript analysis degraded,
Self-MoA failed closed, and validation produced no evidence; no live provider
or model was contacted or loaded.

The same run exposed an adjacent timing risk: the worker can claim a freshly
quick-created Backlog task while the user is still filling and saving its
editor. This run used the pre-save auto-detected user-interviews route even
though the saved editor selected Thematic Analysis. That race remains to be
traced after the notification contract is made truthful. Compass Forge context,
impact, why, and related-file evidence identify the toast, websocket progress
contract, agent execution, Task Editor, and task tests. Next is red-green repair
of the explicit failure outcome before resuming board transitions.

### L-18 | 2026-08-31T06:19:12Z | S2-execute | codex | implementer/verifier

Repaired the realtime terminal-state contract end to end. Task progress now
keeps numeric progress separate from an explicit `outcome`: verified execution
emits `ready_for_review`; failed self-verification emits
`verification_failed`. The legacy and plan execution paths use the same
contract. Frontend classification renders those as `Ready for Review` and
`Task Needs Attention`, falls back to warning for legacy failure text, and
uses neutral `Task Update` for unclassified 100% events rather than inventing
success. Backend `agent_status: warning` is now visible as `Agent Needs
Attention` instead of being silently discarded.

Red-green evidence: the frontend test first failed because the classifier did
not exist, while the backend test failed because `broadcast_task_progress`
rejected `outcome`. Green focused results are 6/6 frontend cases and 1/1
websocket contract case. Broader verification passed 30/30 frontend units,
63/63 websocket/task/agent tests, repository ESLint, production Next build and
TypeScript, 28/28 security controls at 100%, all 86 living feature contracts
with 224 generated artifacts, and `git diff --check`.

The Kanban architecture/researcher contracts now specify that terminal
percentage alone cannot mean completion and that failed verification remains
outside accepted/reportable state. Next is isolated backend/UI publication and
live failure proof. The pre-save quick-create/worker-claim race remains open
and will be traced immediately after that proof.

### L-19 | 2026-08-31T06:35:48Z | S2-execute | codex | verifier/investigator

Published the explicit task-outcome contract in both isolated services. The
running backend image is
`sha256:e175349f375e739b9430274f242880b9a87f0c455a151545166d62debe46d9b2`
and the running UI image is
`sha256:10e2c4a1e2166d284041f7d80d0815132ca28ce36e4164bac43fa17db742af1b`;
both were healthy. Because the QA database lives on backend tmpfs, an online
SQLite backup was checksummed before recreation, streamed into the new
container, restored through SQLite's backup API, and verified as one user, one
project, two documents, and one existing task before browser work resumed.
The exact host and container preservation copies were deleted after proof and
their absence was verified.

The authenticated task journey and uploaded `Interview P1 Sarah` survived the
restore. Two bounded synthetic Thematic Analysis retries were configured from
the real task editor with the existing source and explicit fail-closed
Research Spine instructions. The terminal run reproduced the absent
keychain-secret boundary without contacting or loading a provider/model. At
9.3 seconds after the working notification, the UI rendered amber
`Task Needs Attention` with the verification failure and `Agent Needs
Attention` with the agent warning. It never rendered `Task Complete` or
`Ready for Review`. The database recorded the task as `IN_REVIEW` with
`system_failed`; no research artifact was accepted or made reportable.

The same run sharpened the queued task-state defect. During execution the card
could remain visually in Backlog while the agent reported working and the
database had already advanced it to `IN_REVIEW`; the board did not reconcile
until a later refresh/event. The Review panel's Backlog control also issued
successful task PATCHes but did not visibly return the prior failed task to
Backlog. These observations join the already reproduced pre-save claim race:
execution can see a quick-created task before the editor's skill, instructions,
context, and attachments are atomically saved. Freeze the menu walk here and
trace the create/claim/lock/status/realtime path before testing another menu.

Harness corrections: writing a backup under read-only `/app` and Docker's
direct copy of a tmpfs file were rejected before the successful online-backup
stream; a first read-only count probe had a quoting syntax error and a schema
probe guessed a nonexistent column. None changed product state. No external
exposure, commit, push, merge, PR, or `origin/main` change occurred.

### L-20 | 2026-08-31T13:19:38Z | S2-execute | codex | implementer/verifier

Closed the quick-create/editor-save worker-claim race and the adjacent Kanban
state synchronization defects. Quick-created Backlog tasks can now be created
with an atomic user-owned editing lock, and opening an existing task acquires
the same lock. The main-agent fallback picker now excludes locked tasks, as
the assigned-agent picker already did. A plain Save persists the form while
retaining the lock; Done Editing and review actions save first, release the
lock, then wake the worker. Save failures remain visible and do not close the
editor. Review-target controls now choose `Return to Backlog` or `Resume In
Progress` without mutating state until the user presses `Request Revision`.

Task-progress messages now carry project identity and terminal outcome, and
the project task-realtime subscriber performs a debounced non-destructive
refresh on queue and terminal progress. That makes Backlog, In Progress, In
Review, and terminal metadata reconcile without a manual reload while keeping
the current task list visible during refresh. Living Task Editor, Kanban, and
review contracts were updated and regenerated.

Red-green coverage proved the missing lock, fallback-picker, project-scoped
websocket, realtime refresh, and non-destructive store contracts before the
implementation. Green verification passed 3/3 focused backend lock/websocket
tests, 2/2 focused frontend suites, all 32/32 frontend unit cases, full
repository ESLint, the production Next build and TypeScript, 98/98 graph-
selected task/websocket/agent/project-scope/lifecycle backend tests, all 86
living feature contracts with 224 generated artifacts, the 28/28 security
benchmark at 100%, and `git diff --check`. Focused Ruff reported 137 inherited
findings across the pre-existing large split-agent files; the only introduced
UP017 finding was corrected. Ruff format would rewrite all seven targeted
large files, so that unrelated debt was deliberately not swept into this
change.

Publication exposed and then closed a separate QA-stack authentication defect.
The UI image had been compiled with `localhost` API/WebSocket endpoints while
the documented loopback browser URL was `127.0.0.1`, so password login was
correctly rejected as an untrusted cross-site browser origin. Contract tests
failed red for both frontend images. An initial remediation that left the
runtime endpoint empty was wrong because the shared runtime config preserved
the page's port and tried the backend on 3000; this was detected in the rebuilt
remote image and replaced. Both `qa-frontend` and `qa-ui` now compile canonical
`http://127.0.0.1:8000` and `ws://127.0.0.1:8000` defaults. The QA-stack suite
passed 25/25, Compose rendered quietly with the UI profile, feature docs and
security gates repeated green, and a fresh real password login on
`127.0.0.1:3000` mounted authenticated Settings with the preserved project.

The isolated publication preserved the tmpfs database through an online
SQLite backup at
`/Users/user/istara-qa-testing-20260829-L20-pre.sqlite`, checksum
`75de800796af2e599f0e74087071f2b19f4d661b91fb9b6fc624b02256f97929`,
and restored its one user, one project, three documents, and three tasks before
the browser proof. The current backend image is
`sha256:108843c8951e964ee60662f0bab52d1ffbde64a806b8bf56271a46076f05794d`;
the final canonical-loopback UI image is
`sha256:622bf9a929cf53c4fdcd5b7dc07dfb6e960a64bd7f5831fa41c508807c7319b1`.

The real Kanban acceptance created `Thematic analysis QA edit-lock proof` from
Backlog. Its editor opened immediately with `Locked for editing`; Thematic
Analysis, instructions, context, label, and `Interview P1 Sarah` were entered.
After more than a worker poll cycle the database still showed `BACKLOG`, a
non-null lock, and 0% progress. Save updated the card and persisted the skill
and context while keeping both dialog and lock. Done Editing then closed the
dialog, released the worker, moved the card to In Progress, showed `Agent
Working`, and advanced progress. The deliberately credential-less route
failed closed, made no live provider/model call, stored `IN_REVIEW`, unlocked
the task, recorded the failure, and reconciled the card into In Review without
a page reload. The notification feed retained the corresponding agent warning.
No evidence unit, code, fact, insight, recommendation, or report was accepted.

The review proof entered an explicit human retry instruction, selected Return
to Backlog without prematurely changing the database (`IN_REVIEW` remained
locked), then pressed Request Revision. The editor saved/unlocked/closed, the
card immediately retried in In Progress with `needs revision`, and the second
credential-less attempt again returned to In Review without a reload. The
healthy stack ended at one user, one project, five documents, and four tasks;
the two additional documents are failed-attempt output artifacts and remain
non-reportable. Both the exact host backup and container restore temporary
copy were deleted and their absence was verified after acceptance.

This user journey found three new defects, now queued before another broad
surface: Settings exposes raw `ENC:gAAAA...` ciphertext for the admin email
after the restored database is opened with a different encryption key; reopened
task input/output document chips show truncated UUIDs instead of document
titles; and the next machine failure overwrites the visible human
`what_to_review` instruction rather than preserving it as review history.
Backend logs repeatedly confirm the decryption failure. Investigate the
ciphertext/key lifecycle first as a security-sensitive Settings defect, then
repair document-label resolution and human review-instruction preservation.
No external exposure, commit, push, merge, PR, or `origin/main` change occurred.
...
### L-28 | 2026-09-01T01:00:33Z | S2-execute | codex | channel startup leak closure

The live wizard showed that adapter startup could leak a raw provider/network
exception before the health check boundary. The service now logs that detail
server-side, unregisters failed adapters, marks the instance unhealthy, and
returns the stable actionable connection-check message. Added focused startup
coverage and documented the fail-closed contract in the Integrations >
Messaging architecture page.

Local evidence: focused safe-error checks 2 passed; channel, resilience, and
scope suites 56 passed; feature docs generated 224/check 86; security benchmark
28/28 (100%); `git diff --check` passed. Compass Forge impact/why,
test-impact, suggest-tests, and baseline gate record 444 were captured; the
aggregate remains non-release-ready on inherited debt.

Remote evidence: targeted files were copied to the isolated QA checkout only;
SQLite backup `/Users/user/istara-qa-testing-20260829-L28-pre.sqlite.tar` has
SHA-256 `c907a2197b265c979b4f81266b7c3ba7c124795acde13f57c13f389b0ecce226`.
The rebuilt healthy backend image is
`sha256:f4334ab6563f19841f186c151930285a94a52f952f51df8da1ab3f2eb8774b72`.
Visible browser proof rendered only the stable message for invalid Telegram
credentials, with no `httpx` or token text, zero browser errors/warnings, and
no channel remaining after wizard cleanup. Navigation resumed normally after
dismissing the guided-tour overlay that had been intercepting clicks.

No commit, push, merge, PR, or `origin/main` mutation occurred. Exact next
action: continue loops, steering, multi-turn/long-horizon, agent registry and
A2A, schedule interruption/resume, and remaining menu scenarios.

### L-29 | 2026-09-01T01:03:26Z | S2-execute | codex | loop preview clock characterization

Visible Loops > Schedules testing confirmed the hourly preview is future-only.
The host/browser clock is `Mon Aug 31 22:03 GMT-0300`, so the displayed 23:00
through 03:00 sequence is correct despite crossing midnight. Required Name and
Skill Name fields keep Create disabled until valid input; Cancel exits without
persisting. No implementation change was made because the suspected defect was
not reproducible after timezone verification.

No remote rebuild, commit, push, merge, PR, or `origin/main` mutation occurred.
Exact next action: test cron presets and validation, then Agent Loops, Custom,
History, Chat steering/files/audio, and multi-turn flows.

### L-30 | 2026-09-01T01:18:40Z | S2-execute | codex | Agent Loops scope closure (local red/green)

Agent Loops rendered universal system agents from the general registry as if
they were project-owned loops. A browser click on Pause for `Istara` sent the
project-scoped pause route and received `404 Not Found`; the card stayed
unchanged and the QA database remained `IDLE`. The same path exposed an
interval Apply affordance that could only update local state when the backend
mutation was out of scope. The backend route is intentionally fail-closed, so
the defect was the frontend affordance mismatch.

`AgentLoopsTab` now joins the project-scoped loop configurations with registry
metadata and renders mutable cards only for configs returned for the active
project. Universal agents remain visible in the Agents registry but are absent
from the mutable Agent Loops list; the empty state explains the boundary. A
focused helper regression covers exclusion and retention, and the feature
architecture/researcher docs record the behavior.

Local evidence: red helper test failed before implementation; green helper test
2 passed; component/test ESLint passed; `npm run build` compiled and completed
TypeScript; loop and project-scope Python tests 46 passed; feature docs generated
224/check 86; `git diff --check` passed. The unsupported `npm run typecheck`
script was recorded as a command mismatch, not a code failure.

Compass Forge impact/why/test-impact/suggest-tests were run for the component;
the aggregate gate remains non-release-ready on inherited debt. No remote
rebuild, commit, push, merge, PR, or `origin/main` mutation occurred.

Exact next action: publish the targeted UI/test/docs slice to isolated QA,
rebuild, verify that no Pause/Resume/Apply controls or 404 requests remain for
universal agents, then continue Custom/History and Chat scenarios.
...
`/Users/user/istara-qa-testing-20260829` and rebuilt with
`QA_RUN_ID=testing-20260829`. The resulting UI image digest is
`sha256:1487abcb2c12dcd01be58b978069cbacbfaedf3571921a12ff4b40eecd92b7d`.
...
namespace was accidentally started when `QA_RUN_ID` was omitted; it is not
part of this evidence and must not be used for acceptance claims.

In a fresh browser tab, the QA user logged in through the visible flow,
...
intercepting navigation. Loops > Agent Loops now renders
`No project-scoped agent loops found.` and
...
pause/resume/PATCH mutation requests and therefore no 404 path. The isolated
QA update-check DNS warning is expected no-egress behavior and remained an
update-unavailable status rather than a raw exception.

Compass Forge remains non-release-ready on inherited complexity, route/type
drift, and secret-flow debt. No commit, push, merge, PR, or `origin/main`
mutation occurred. The temporary database restore and guided-tour dismissal
are recorded so the next agent can reproduce the state safely.

Exact next action: exercise Custom and History loop tabs in the correct
...
`Agent Loop Scope QA` project. Missing required fields kept Create disabled.
Selecting `research-synthesis`, entering a name and description, and using
the safe non-running interval of `86400` seconds enabled Create; the visible
result showed `QA validation loop`, `86400s interval`, and `active`. Backend
logs confirmed `OPTIONS /api/loops/custom` 200 and `POST /api/loops/custom`
201, followed by a health refresh; no model execution was triggered. The form
reset cleanly after creation.

Cron Expression mode rendered all six presets and five field selectors.
Choosing `Every Monday` produced `0 9 * * 1`, `Every Mon at 9:00`, and five
future runs from Sep 7 onward in the browser's local timezone. Cancel closed
the form without persisting a second loop. The earlier blank/zero interval
input was rejected by the HTML `min=60` control and did not create a request.

History loaded its empty state and exposed type/status/date filters. Selecting
Scheduled + Failure and the bounded 2026-01-01 through 2026-01-02 range, then
clicking Filter, kept `No execution history yet.` Backend logs confirmed the
project-scoped executions endpoint responded to preflight and GET requests;
no 4xx/5xx was observed, and pagination was absent for zero results.
...
The model menu opened with two enabled QA-contract models and all other
catalog entries disabled with `Configure in Settings`; selecting the QA chat
stub kept the visible core/model chips in sync. A first turn and a second turn
were submitted with effort set to `On`. Both rendered the user messages and
the stable actionable response that this deployment uses a QA contract stub,
not a real model, and requires `OLLAMA_HOST` or a configured Pi endpoint. The
backend logged `POST /api/chat` 200 and the explicit stub rejection, with no
raw exception or 4xx/5xx response.

The project-document picker opened, searched a nonexistent title, and closed
cleanly with `No documents found`; the upload button opened the native chooser
without changing the page when no file was selected. The voice button
attempted microphone access and produced the visible `Voice Error` toast,
`Could not access microphone. Please check permissions.` The input returned to
idle and emitted no recording/transcription request. The effort selector
changed to `On`, and the header chip updated. Session history retained both
turns and the stable unavailable response after navigation.

Because this isolated lane has no live provider, no real model answer,
streaming tool call, or audio transcription is claimed. No implementation
change was justified. No commit, push, merge, PR, or `origin/main` mutation
occurred. Compass Forge remains non-release-ready on inherited complexity,
route/type drift, and secret-flow debt.
...
configuration, Integrations overview/messaging/surveys/deployments/MCP
