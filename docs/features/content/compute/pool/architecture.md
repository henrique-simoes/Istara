...
- The canonical three-model real-user benchmark is fully Docker-owned: the Compose project runs Istara, the admin flow, and the first donor, while disposable Docker containers run the simulated researchers plus the second and third llama.cpp donor endpoints through the nested Docker socket. The Mac Studio is only the Docker host and SSH control plane; the historical host-managed topology is refused before any live service, model, or package operation. The wrapper requires pre-provisioned model files under a configured read-only root, removes benchmark-owned donor/client containers after the run unless explicit keep/debug flags are set, and never downloads weights, installs host packages, manages Colima, or starts a host-managed Istara process. It removes stale benchmark-owned server-sandbox containers before host health checks, starts only donors whose model endpoint preflight succeeds, expects all three project-scoped relay nodes, waits for all three to be healthy immediately before Research Spine coding, and requires coding to use three distinct model coders across three distinct served donor routes instead of a one-coder fallback or one-server alias set.
