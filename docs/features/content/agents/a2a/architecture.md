...
code_references: ["frontend/src/components/agents/AgentsView.tsx", "frontend/src/stores/agentStore.ts", "frontend/src/lib/api.ts", "backend/app/api/routes/agents.py", "backend/app/api/agent_project_scope.py", "backend/app/api/routes/a2a.py", "backend/app/api/websocket.py", "backend/app/services/a2a.py", "backend/app/models/agent.py", "backend/alembic/versions/019_a2a_message_project_scope.py", "backend/app/core/agent_lifecycle.py", "backend/app/core/pi_runtime/seams.py", "backend/app/core/pi_runtime/idempotency.py", "backend/app/models/pi_tool_execution.py", "backend/alembic/versions/032_pi_tool_executions.py", "backend/app/core/sub_agent_worker.py", "backend/app/skills/system_actions.py"]
...
test_references: ["tests/test_agents.py", "tests/test_a2a_project_claims.py", "tests/test_a2a_service_scope.py", "tests/test_a2a_security.py", "tests/test_websocket.py", "tests/test_project_scope_contracts.py", "tests/pi_production/test_pi_tool_idempotency.py"]
...
- A Pi-selected delegation whose terminal is `error` or `aborted` fails closed before a response is sent or persisted, even when the worker streamed partial text; the failed turn produces no A2A reply artifact.
- A Pi-selected A2A delegation carries the persisted `A2AMessage.id` as its request identity into the authority seam. Mutating tool calls are keyed by project, tool name, and canonical argument digest; completed outcomes replay after a worker restart, while an unsettled row blocks automatic re-execution with `tool_recovery_required`. This is a recovery barrier, not an exactly-once guarantee: operators must reconcile an unknown side effect before retrying.
...
- `tests/test_agents.py`, `tests/test_a2a_project_claims.py`, `tests/test_a2a_service_scope.py`, `tests/test_a2a_security.py`, `tests/test_websocket.py`, `tests/test_project_scope_contracts.py`, `tests/test_pi_replacement_candidate.py`, `tests/pi_production/test_scenario_delegation.py`, and `tests/pi_production/test_pi_tool_idempotency.py` verify that project-scoped A2A logs exclude unrelated project/global messages, service writes and mark-read mutations require project scope, conflicting metadata/task/row/agent project claims are dropped, JSON-RPC task writes and discovery cannot proceed without project scope, background inbox/thread helpers do not mix projects, realtime delivery resolves only consistent project claims, an admitted Pi-selected `tasks/send` envelope reaches the normal inbox dispatcher before any Pi delegation runs, and mutation replay/recovery barriers preserve the persisted message identity.
- `tests/pi_production/test_seams_fail_closed.py` verifies error and abort terminals, including failures after partial output, cannot send or persist a Pi A2A response.
