...
- The Petals OpenAI compatibility stream uses the donor's receipt-preserving chat seam and emits the provider `usage` object on its terminal chunk when available. Missing usage is marked as a complete text estimate, and the final route evidence exposes `usage_estimate`; a streamed bridge response must never present a partial provider ledger as exact.
