...
- Legacy compatibility fixtures and any manually inserted `CodeApplication` that have accepted/reconciled row flags but no `coding_run_id` remain an explicit architecture-debt boundary: the current task validity gate can treat them as reportable when their task is human-approved Done. New Research Spine production paths must persist a governed coding run and should not rely on this legacy exception; a migration/compatibility decision is still required before tightening the gate globally.
