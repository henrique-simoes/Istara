...
- W8 migrated `validation.py:_get_embeddings` to `agentic.embed`: the legacy engine keeps the unchanged embedding plane and the Pi engine uses the `EmbeddingsGateway`. The W7 validation migration therefore has no separate embedding fallback branch to document.
...
- The concrete independence unit is the serving endpoint identity. Same-model endpoints are valid independent raters only when their endpoint identities are distinct and preserved in route evidence; a missing or insufficient endpoint set is lower assurance and remains unavailable/blocked.
